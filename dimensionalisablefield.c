...
PetscErrorCode DimensionalisableFieldCreate(DimensionalisableField *pf, DM dm, PetscScalar *scalings, PetscBool isScaled)
{
  ...
  ierr = PetscMalloc1(1, pf);
  CHKERRQ(ierr);
  f = *pf;

  f->dm = dm;

  ierr = DMGetType(f->dm, &dmtype);
  CHKERRQ(ierr);
  ierr = PetscStrcmp(dmtype, DMCOMPOSITE, &isComposite);
  CHKERRQ(ierr);
  if (isComposite)
  {
    ierr = DMCompositeGetNumberDM(f->dm, &(f->numDomains));
    CHKERRQ(ierr);
  }
  else
  {
  ...
  ierr = PetscMalloc1(f->numDomains, &f->scaling);
  CHKERRQ(ierr);
  ...
  ierr = DMCreateGlobalVector(f->dm, &f->vecGlobal);
  CHKERRQ(ierr);

  f->name = "Unnamed DimensionalisableField";

  ierr = PetscMalloc1(f->numDomains, &f->subdomainNames);
  CHKERRQ(ierr);
  ierr = PetscMalloc1(f->numDomains, &f->subdomainUnits);
  CHKERRQ(ierr);
  for (i = 0; i < f->numDomains; ++i)
  {
...
PetscErrorCode DimensionalisableFieldDestroy(DimensionalisableField *pf)
{
  ...
  if (f->vecGlobal)
  {
    ierr = VecDestroy(&f->vecGlobal);
    CHKERRQ(ierr);
  }
  ierr = PetscFree(f->scaling);
  CHKERRQ(ierr);
  ierr = PetscFree(f->subdomainNames);
  CHKERRQ(ierr);
  ierr = PetscFree(f->subdomainUnits);
  CHKERRQ(ierr);
  ierr = PetscFree(*pf);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
...
PetscErrorCode DimensionalisableFieldDuplicate(DimensionalisableField f, DimensionalisableField *pfNew)
{
  ...
  PetscFunctionBeginUser;
  ierr = DimensionalisableFieldCreate(pfNew, f->dm, f->scaling, f->scaled);
  CHKERRQ(ierr);
  fNew = *pfNew;
  ierr = DimensionalisableFieldSetName(fNew, f->name);
  CHKERRQ(ierr);
  for (i = 0; i < fNew->numDomains; ++i)
  {
    ierr = DimensionalisableFieldSetSubdomainName(fNew, i, f->subdomainNames[i]);
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldSetSubdomainUnits(fNew, i, f->subdomainUnits[i]);
    CHKERRQ(ierr);
  }
  PetscFunctionReturn(0);
}
...
PetscErrorCode DimensionalisableFieldCreateLocalVec(DimensionalisableField f, Vec *pv)
{
  ...
  PetscFunctionBeginUser;
  ierr = DMCreateLocalVector(f->dm, pv);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
static PetscErrorCode DimensionalisableFieldScaleOrUnscale(DimensionalisableField f, PetscBool forward)
{
  ...
  else
  {
    ...
    if (f->numDomains == 1)
    {
      if (forward)
      {
        ierr = VecScale(f->vecGlobal, *f->scaling);
        CHKERRQ(ierr);
      }
      else
      {
        ierr = VecScale(f->vecGlobal, 1.0 / (*f->scaling));
        CHKERRQ(ierr);
      }
    }
    else
    {
      ierr = PetscMalloc1(f->numDomains, &subVecs);
      CHKERRQ(ierr);
      ierr = DMCompositeGetAccessArray(f->dm, f->vecGlobal, f->numDomains, NULL, subVecs);
      CHKERRQ(ierr);
      for (i = 0; i < f->numDomains; ++i)
      {
        if (forward)
        {
          ierr = VecScale(subVecs[i], f->scaling[i]);
          CHKERRQ(ierr);
        }
        else
        {

          ierr = VecScale(subVecs[i], 1.0 / (f->scaling[i]));
          CHKERRQ(ierr);
        }
      }
      ierr = DMCompositeRestoreAccessArray(f->dm, f->vecGlobal, f->numDomains, NULL, subVecs);
      CHKERRQ(ierr);
...
PetscErrorCode DimensionalisableFieldScale(DimensionalisableField f)
{
  PetscErrorCode ierr;
  PetscFunctionBeginUser;
  ierr = DimensionalisableFieldScaleOrUnscale(f, PETSC_TRUE);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode DimensionalisableFieldUnscale(DimensionalisableField f)
{
  PetscErrorCode ierr;
  PetscFunctionBeginUser;
  ierr = DimensionalisableFieldScaleOrUnscale(f, PETSC_FALSE);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode valueToString(PetscScalar val, char *str, int maxLength)
{
  ...
  if (ierr >= maxLength || ierr < 0)
    SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_LIB, "quadmath_snprintf() failed");
#else
  ierr = PetscSNPrintf(str, maxLength, "%16.16g", val);
  CHKERRQ(ierr);
#endif
  return 0;
}
...
PetscErrorCode DimensionalisableFieldToJSON(DimensionalisableField const f, cJSON **pjson)
{
  ...
  ierr = DimensionalisableFieldGetGlobalVec(f, &vec);
  CHKERRQ(ierr);
  ...
  *pjson = cJSON_CreateObject();
  ...
  if (f->numDomains > 1)
  {
    ...
    ierr = PetscMalloc1(f->numDomains, &subVecs);
    CHKERRQ(ierr);
    ierr = DMCompositeGetAccessArray(f->dm, vec, f->numDomains, NULL, subVecs);
    CHKERRQ(ierr);
  }
  else
  {
  ...
  for (d = 0; d < f->numDomains; ++d)
  {
    ...
    /* Current implementation assumes single rank */
    {
      ...
      if (size != 1)
        SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_SUP, "Not implemented in parallel");
    }
    ierr = VecGetArrayRead(vecCurr, &arr);
    CHKERRQ(ierr);
    values = cJSON_CreateArray();
    {
      ...
      {
        char str[FORMAT_STRING_SIZE];
        ierr = valueToString(arr[i], str, FORMAT_STRING_SIZE);
        CHKERRQ(ierr);
        cJSON_AddItemToArray(values, cJSON_CreateString(str));
      }
    }
    ierr = VecRestoreArrayRead(vecCurr, &arr);
    CHKERRQ(ierr);
  ...
  if (f->numDomains > 1)
  {
    ...
    ierr = DMCompositeRestoreAccessArray(f->dm, vec, f->numDomains, NULL, subVecs);
    CHKERRQ(ierr);
    ierr = PetscFree(subVecs);
    CHKERRQ(ierr);
  }
...
PetscErrorCode DimensionalisableFieldSetUnits(DimensionalisableField f, const char *units)
{
  PetscFunctionBeginUser;
  if (f->numDomains != 1)
    SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_ARG_WRONG, "Not supported for multiple-subdomain DimensionalisableField objects. Use DimensionalisableFieldSetSubdomainUnits");
...
PetscErrorCode JSON_add_single_value_to_object(DM dm, PetscScalar scaling, const char *name, const char *units, const PetscScalar value, cJSON *data)
{
  ...
  ierr = DimensionalisableFieldCreate(&dfield, dm, &scaling, PETSC_FALSE);
  CHKERRQ(ierr);
  ierr = DimensionalisableFieldSetName(dfield, name);
  CHKERRQ(ierr);
  ierr = DimensionalisableFieldSetUnits(dfield, units);
  CHKERRQ(ierr);
  ierr = VecSetValue(dfield->vecGlobal, 0, value, INSERT_VALUES);
  CHKERRQ(ierr);
  ierr = VecAssemblyBegin(dfield->vecGlobal);
  CHKERRQ(ierr);
  ierr = VecAssemblyEnd(dfield->vecGlobal);
  CHKERRQ(ierr);
  ierr = DimensionalisableFieldToJSON(dfield, &item);
  CHKERRQ(ierr);
  cJSON_AddItemToObject(data, name, item);
  ierr = DimensionalisableFieldDestroy(&dfield);
  CHKERRQ(ierr);
