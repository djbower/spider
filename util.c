...
PetscErrorCode set_d_dr( Ctx *E )
{
    /* create a matrix containing the 2nd order accurate stencil for
       ...
       basic nodes.  Currently this matrix is global */
    ...
    PetscScalar value[3];
    PetscScalar *arr_radius_s, *arr_radius_b, dh, h1, h2, denom;
    Mat A;
    Mesh *M;
...
// TODO: debug output below is old (matrix size is wrong)
/*#if (defined DEBUGOUTPUT)
    ierr = PetscPrintf(PETSC_COMM_WORLD,"set_d_dr : creating a matrix of size %D\n",numpts_s);CHKERRQ(ierr);
    {
       PetscMPIInt rank;
       ierr = MPI_Comm_rank(PETSC_COMM_WORLD,&rank);CHKERRQ(ierr);
       ierr = PetscPrintf(PETSC_COMM_WORLD,"[%D] set_d_dr : local rows %d<=i<%d\n",rank,ilo_s,ihi_s);CHKERRQ(ierr);
    }
#endif*/
    ...
    for (i=rstart; i<rend; i++) {
        ...
        // distance of basic node from (first) staggered node
        dh = 0.5 * (arr_radius_b[i] - arr_radius_b[i-1]);
        col[0]=i-1; col[1]=i; col[2]=i+1;
        value[0] = (2.0*(dh-h1)-h2) / (h1*(h1+h2));
        value[1] = (-2.0*dh+h1+h2) / (h1*h2);
        value[2] = (2.0*dh-h1) / (h2*(h1+h2));
        ierr = MatSetValues(A,1,&i,3,col,value,INSERT_VALUES);CHKERRQ(ierr);
    }

    /* need backward difference for last point, to retain ability to dot
       product with half the basic node spacing when sequentially stepping
       forward */
    /* TODO FIXME NOT GIVING RIGHT ANSWER AT MOMENT */
    h1 = arr_radius_s[numpts_s-3] - arr_radius_s[numpts_s-4];
    h2 = arr_radius_s[numpts_s-2] - arr_radius_s[numpts_s-3];
    dh = 0.5 * (arr_radius_b[numpts_s-2] - arr_radius_b[numpts_s-3]);
    col[0]=numpts_s-3; col[1]=numpts_s-2; col[2]=numpts_s-1;
    value[0] = (2.0*dh-h2) / (h1*(h1+h2));
    value[1] = (-2.0*dh+h2-h1) / (h1*h2);
    value[2] = (2.0*dh+h1)/(h2*(h1+h2));
    i = numpts_b-2;
    ierr = MatSetValues(A,1,&i,3,col,value,INSERT_VALUES);CHKERRQ(ierr);
