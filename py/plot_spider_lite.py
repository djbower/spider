...
class MyJSON( object ):
    ...
    def __set_time_list_from_files_in_directory( self ):
        ...
        try:
            file_l = [f for f in os.listdir(self.indir) if os.path.isfile(os.path.join(self.indir,f))]
...
def set_xaxis_from_kwargs( ax, myjson_o=None, **kwargs ):
    ...
    if xaxis == 'time':
        ...
        if myjson_o is not None:
            myjson_o.xdata = myjson_o.get_values( ['data','pressure_b'], squeeze=False, **kwargs )[:,0]
        ...
        if myjson_o is not None:
            myjson_o.xdata = myjson_o.get_values( ['data','pressure_s'], squeeze=False, **kwargs )[:,0]
            myjson_o.xdata *= 1.0E-9 # to GPa
        xticks = (0,50,100,135)
        ax.set_xticks( xticks )
        ax.set_xlim( 0, 138 )
        ...
        if myjson_o is not None:
            myjson_o.xdata = myjson_o.get_values( ['rheological_front_phi','phi_global'] )
