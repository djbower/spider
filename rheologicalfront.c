...
PetscInt get_crossover_index(DM dm, const Vec vec, PetscScalar value, PetscInt offset)
{
    ...
    ierr = DMDAVecGetArrayRead(dm, vec, &arr_vec);
    CHKERRQ(ierr);
    ...
    ierr = DMDAVecRestoreArrayRead(dm, vec, &arr_vec);
    CHKERRQ(ierr);
...
PetscErrorCode JSON_add_rheological_front(DM dm, ScalingConstants C, RheologicalFront *Rf, const char *name, cJSON *json)
{
    ...
    ierr = JSON_add_single_value_to_object(dm, 1, "mesh_index", "None", Rf->mesh_index, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, C->RADIUS, "depth", "m", Rf->depth, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, C->PRESSURE, "pressure", "Pa", Rf->pressure, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, C->TEMP, "temperature", "K", Rf->temperature, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, 1.0, "phi_global", "None", Rf->phi_global, data);
    CHKERRQ(ierr);

    ierr = JSON_add_rheological_front_mantle_properties(dm, C, &Rf->above_middle, "above_middle", data);
    CHKERRQ(ierr);
    ierr = JSON_add_rheological_front_mantle_properties(dm, C, &Rf->above_mass_avg, "above_mass_avg", data);
    CHKERRQ(ierr);
    ierr = JSON_add_rheological_front_mantle_properties(dm, C, &Rf->below_middle, "below_middle", data);
    CHKERRQ(ierr);
    ierr = JSON_add_rheological_front_mantle_properties(dm, C, &Rf->below_mass_avg, "below_mass_avg", data);
    CHKERRQ(ierr);
...
static PetscErrorCode JSON_add_rheological_front_mantle_properties(DM dm, ScalingConstants C, RheologicalFrontMantleProperties *Rfmp, char const *name, cJSON *json)
{
    ...
    data = cJSON_CreateObject();
    ierr = JSON_add_single_value_to_object(dm, 1.0, "phi", "None", Rfmp->phi, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, C->RADIUS, "depth", "m", Rfmp->depth, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, C->PRESSURE, "pressure", "Pa", Rfmp->pressure, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, C->TEMP, "temperature", "K", Rfmp->temperature, data);
    CHKERRQ(ierr);
    cJSON_AddItemToObject(json, name, data);
