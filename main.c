...
int main(int argc, char **argv)
{
  ...
  /* Print options file(s) being used. If no options file is specified, use a default */
  {
    ...
    for (int i = 0; i < argc; ++i)
    {
      ...
      ierr = PetscStrcmp(argv[i], "-options_file", &detected);
      CHKERRQ(ierr);
    ...
    {
      if (argc > 1)
        SETERRQ1(PETSC_COMM_WORLD, PETSC_ERR_SUP, "If you supply any options, you must supply an options file explicitly. For the default, use \n   -options_file %s", default_options_filename);
      ierr = MakeRelativeToSourcePathAbsolute(default_options_filename);
      CHKERRQ(ierr);
      ierr = PetscPrintf(PETSC_COMM_WORLD, "Using default options file: %s\n", default_options_filename);
      CHKERRQ(ierr);
      ierr = PetscOptionsInsertFile(PETSC_COMM_WORLD, NULL, default_options_filename, PETSC_TRUE);
      CHKERRQ(ierr);
    }
  }
  ...
  ierr = PrintFields(&ctx);
  CHKERRQ(ierr);

  /* Create Vector for solution provided to timestepper */
  ierr = DimensionalisableFieldGetGlobalVec(ctx.solDF, &sol);
  CHKERRQ(ierr);

  /* see ic.c : must call after setup_ctx */
  ierr = set_initial_condition(&ctx, sol);
  CHKERRQ(ierr);
  ...
  ierr = TSSundialsSetTolerance(ts, 1.0e-14, 1.0e-14);
  CHKERRQ(ierr);
#else
  ierr = TSSundialsSetTolerance(ts, 1.0e-6, 1.0e-6);
  CHKERRQ(ierr);
#endif
     ...
     macro step takes a longer amount of time. */
  {
    ...
    ierr = TSMonitorSet(ts, TSMonitorWalltimed, &mctx, NULL);
    CHKERRQ(ierr);
    ...
    ierr = PetscPrintf(PETSC_COMM_WORLD, "*** Starting at t0 = %f, Will perform %D macro (output) steps of length %f = %f years\n",
                       P->t0, P->nstepsmacro, (double)P->dtmacro, (double)(P->dtmacro * (*P->scaling_constants).TIMEYRS));
    CHKERRQ(ierr);
    time = P->t0;
    ...
    ierr = TSSetMaxSteps(ts, P->maxsteps);
    CHKERRQ(ierr);
    ierr = TSSetMaxTime(ts, nexttime);
    CHKERRQ(ierr);
    ...
    if (P->monitor)
    {
      ierr = TSCustomMonitor(ts, stepmacro, time, sol, &ctx, &mctx);
      CHKERRQ(ierr);
    }
    ...
    if (P->rollBackActive)
    {
      ierr = TSRollBackGenericActivate(ts);
      CHKERRQ(ierr);
    }
    if (P->postStepActive)
    {
      ierr = TSSetApplicationContext(ts, &ctx);
      CHKERRQ(ierr);
      ierr = PostStepDataInitialize(&ctx);
      CHKERRQ(ierr); /* use initial condition here */
    ...
    for (stepmacro = P->stepmacro + 1; stepmacro <= P->nstepsmacro; ++stepmacro)
    {
      ...
      {
        ierr = TSView(ts, PETSC_VIEWER_STDOUT_WORLD);
        CHKERRQ(ierr);
      }
      ierr = TSGetSolveTime(ts, &time);
      CHKERRQ(ierr);
      if (P->monitor)
      {
        ierr = TSCustomMonitor(ts, stepmacro, time, sol, &ctx, &mctx);
...
PetscErrorCode PrintSPIDERHeader()
{
  ...
  ierr = PetscPrintf(
      ...
      SPIDER_MAJOR_VERSION, SPIDER_MINOR_VERSION, SPIDER_PATCH_VERSION);
  CHKERRQ(ierr);
#if defined(PETSC_USE_REAL___FLOAT128)
...
PetscErrorCode PrintFields(Ctx *ctx)
{
  ...
  CHKERRQ(ierr);
  ierr = PetscMalloc1(ctx->numFields, &sub_dms);
  CHKERRQ(ierr);
  ierr = DMCompositeGetEntriesArray(ctx->dm_sol, sub_dms);
  CHKERRQ(ierr);
  for (f = 0; f < ctx->numFields; ++f)
  {
    ...
    ierr = DMGetGlobalVector(sub_dms[f], &vecTemp);
    CHKERRQ(ierr);
    ierr = VecGetSize(vecTemp, &nEntries);
    CHKERRQ(ierr);
    entriesString = nEntries > 1 ? "entries" : "entry";
    ierr = PetscPrintf(PETSC_COMM_WORLD, "  %D: %s (%D %s)\n", f, SpiderSolutionFieldDescriptions[ctx->solutionFieldIDs[f]], nEntries, entriesString);
    CHKERRQ(ierr);
    ierr = DMRestoreGlobalVector(sub_dms[f], &vecTemp);
    CHKERRQ(ierr);
  }
  ierr = PetscFree(sub_dms);
  CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD, "\n");
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
