...
PetscErrorCode EOSSetUpFromOptions_AdamsWilliamson(EOS eos, const char *prefix, const FundamentalConstants FC, const ScalingConstants SC)
{
  ...
  ierr = PetscOptionsGetPositiveScalar("-coresize", &data->radius_core, 0.55, NULL);
  ...
  data->gravity = -10.0;
  ierr = PetscOptionsGetScalar(NULL, NULL, "-gravity", &data->gravity, NULL);
  CHKERRQ(ierr);
  data->gravity /= SC->GRAVITY;

  /* surface density (kg/m^3) */
  ierr = PetscSNPrintf(buf, sizeof(buf), "%s%s%s", "-", prefix, "_rhos");
  CHKERRQ(ierr);
  ierr = PetscOptionsGetPositiveScalar(buf, &data->density_surface, 4078.95095544, NULL);
  CHKERRQ(ierr);
  data->density_surface /= SC->DENSITY;

  /* parameter (1/m) */
  ierr = PetscSNPrintf(buf, sizeof(buf), "%s%s%s", "-", prefix, "_beta");
  CHKERRQ(ierr);
  ierr = PetscOptionsGetPositiveScalar(buf, &data->beta, 1.1115348931000002e-07, NULL);
  CHKERRQ(ierr);
  data->beta *= SC->RADIUS;

  /* we need the average density for the mass coordinate to radius mapping */
  ierr = EOSAdamsWilliamson_GetMassCoordinateAverageRho(data, &data->density_average);
  CHKERRQ(ierr);
...
PetscErrorCode EOSCreate_AdamsWilliamson(EOS eos)
{
  ...
  ierr = PetscMalloc1(1, (data_EOSAdamsWilliamson **)(&eos->impl_data));
  CHKERRQ(ierr);
...
PetscErrorCode EOSAdamsWilliamsonGetPressureFromRadius(const EOS eos, PetscScalar R, PetscScalar *P_ptr)
{
  ...
  ierr = EOSCheckType(eos, SPIDER_EOS_ADAMSWILLIAMSON, &is_adams);
  CHKERRQ(ierr);
  if (!is_adams)
    SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Must be called on an Adams-Williamson EOS");

  ierr = EOSAdamsWilliamson_GetPressureFromRadius(adams, R, P_ptr);
  CHKERRQ(ierr);
...
static PetscErrorCode EOSAdamsWilliamson_GetMassWithinShell(const data_EOSAdamsWilliamson *adams, PetscScalar Rout, PetscScalar Rin, PetscScalar *mass_ptr)
{
  ...
  ierr = EOSAdamsWilliamson_GetMassWithinRadius(adams, Rout, &massout);
  CHKERRQ(ierr);
  ierr = EOSAdamsWilliamson_GetMassWithinRadius(adams, Rin, &massin);
  CHKERRQ(ierr);
...
PetscErrorCode EOSAdamsWilliamsonGetMassWithinShell(const EOS eos, PetscScalar Rout, PetscScalar Rin, PetscScalar *mass_ptr)
{
  ...
  ierr = EOSAdamsWilliamson_GetMassWithinShell(adams, Rout, Rin, mass_ptr);
  CHKERRQ(ierr);
...
static PetscErrorCode EOSAdamsWilliamson_GetMassCoordinateAverageRho(const data_EOSAdamsWilliamson *adams, PetscScalar *rho_ptr)
{
  ...
  ierr = EOSAdamsWilliamson_GetMassWithinShell(adams, adams->radius, adams->radius_core, &mass);
  CHKERRQ(ierr);
...
PetscErrorCode EOSAdamsWilliamson_ObjectiveFunctionRadius(SNES snes, Vec x, Vec f, void *ptr)
{
  ...
  CHKERRQ(ierr); /* initial guess of r */
  /* below instead should use composite DM */
  ierr = VecGetArrayRead(M->xi_b, &xi_b);
  CHKERRQ(ierr); /* target mass coordinate */
  ierr = VecGetArrayRead(M->xi_s, &xi_s);
  CHKERRQ(ierr);
  ierr = VecGetArray(f, &ff);
  CHKERRQ(ierr); /* residual function */

  for (i = 0; i < numpts_b + numpts_s; ++i)
  {
    /* mass within radial shell of mantle */
    ierr = EOSAdamsWilliamson_GetMassWithinShell(adams, xx[i], adams->radius_core, &ff[i]);
    CHKERRQ(ierr);
  ...
  CHKERRQ(ierr);
  ierr = VecRestoreArrayRead(M->xi_b, &xi_b);
  CHKERRQ(ierr);
  ierr = VecRestoreArrayRead(M->xi_s, &xi_s);
  CHKERRQ(ierr);
...
PetscErrorCode EOSAdamsWilliamson_JacobianRadius(SNES snes, Vec x, Mat jac, Mat B, void *ptr)
{
  ...
  CHKERRQ(ierr);
  ierr = VecSetSizes(diag, PETSC_DECIDE, numpts_b + numpts_s);
  CHKERRQ(ierr);
  ...
  ierr = VecGetArrayRead(x, &xx);
  CHKERRQ(ierr);
  ierr = VecGetArray(diag, &arr_diag);
  CHKERRQ(ierr);

  /* set diagonal */
  for (i = 0; i < numpts_b + numpts_s; ++i)
  {
    ierr = EOSAdamsWilliamson_GetMassElement(adams, xx[i], &arr_diag[i]);
    CHKERRQ(ierr);
    arr_diag[i] *= 3.0 / adams->density_average;
    ierr = EOSAdamsWilliamson_GetMassWithinShell(adams, xx[i], adams->radius_core, &mass);
    CHKERRQ(ierr);
  ...
  ierr = VecRestoreArrayRead(x, &xx);
  CHKERRQ(ierr);
  ierr = VecRestoreArray(diag, &arr_diag);
  CHKERRQ(ierr);

  ierr = MatDiagonalSet(B, diag, INSERT_VALUES);
  CHKERRQ(ierr);
...
PetscErrorCode EOSAdamsWilliamsonMassCoordinateSpatialDerivative(const EOS eos, PetscScalar R, PetscScalar xi, PetscScalar *dxidr_ptr)
{
  ...
  ierr = EOSCheckType(eos, SPIDER_EOS_ADAMSWILLIAMSON, &is_composite);
  CHKERRQ(ierr);
  if (!is_composite)
    SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Must be called on an Adams-Williamson EOS");
