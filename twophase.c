...
PetscErrorCode set_Mliq(Ctx *E)
{
    ...
    // Mliq = sum[ phi*dm ]
    ierr = VecDuplicate(S->phi_s, &mass_s);
    CHKERRQ(ierr);
    ierr = VecCopy(S->phi_s, mass_s);
    CHKERRQ(ierr);
    ierr = VecPointwiseMult(mass_s, mass_s, M->mass_s);
    CHKERRQ(ierr);
...
PetscErrorCode set_Msol(Ctx *E)
{
    ...
    ierr = VecCopy(S->phi_s, mass_s);
    CHKERRQ(ierr);
    ierr = VecScale(mass_s, -1.0);
    CHKERRQ(ierr);
    ierr = VecShift(mass_s, 1.0);
    CHKERRQ(ierr);
    ierr = VecPointwiseMult(mass_s, mass_s, M->mass_s);
    CHKERRQ(ierr);
...
PetscErrorCode set_dMliqdt(Ctx *E)
{
...
#if defined(PETSC_USE_DEBUG)
    {
        ...
        ierr = EOSCheckType(P->eos, SPIDER_EOS_COMPOSITE, &is_composite);
        CHKERRQ(ierr);
        if (!is_composite)
            SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_ARG_OUTOFRANGE, "Only defined for a composite EOS");
    }
#endif
...
PetscErrorCode set_rheological_front(Ctx *E)
{
    ...
    ierr = DMDAGetInfo(da_s, NULL, &numpts_s, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
    CHKERRQ(ierr);

    /* critical melt fraction */
    index = get_crossover_index(da_s, S->phi_s, P->phi_critical, 0);
    ierr = make_vec_mask(da_s, index, &mask_s);
    CHKERRQ(ierr);
    ierr = set_rheological_front_mantle_properties(E, Rfp, index, &mask_s);
    CHKERRQ(ierr);
    ierr = VecDestroy(&mask_s);
    CHKERRQ(ierr);
    ...
    ierr = make_vec_mask(da_s, numpts_s, &mask_s);
    CHKERRQ(ierr);
    ierr = average_by_mass_staggered(E, S->phi_s, &mask_s, &phi_global);
    CHKERRQ(ierr);
...
static PetscErrorCode set_rheological_front_mantle_properties(Ctx *E, RheologicalFront *Rf, PetscInt index, Vec *mask_ptr_s)
{
    ...
    Rf->mesh_index = index;
    ierr = VecGetValues(M->radius_b, 1, &index, &radius);
    CHKERRQ(ierr);
    ...
    ierr = VecGetValues(S->phi, 1, &index_above, &phi);
    CHKERRQ(ierr);
    Rf->above_middle.phi = phi;
    ierr = VecGetValues(M->radius_b, 1, &index_above, &radius);
    CHKERRQ(ierr);
    ...
    /* average by mass */
    ierr = average_by_mass_staggered(E, S->phi_s, mask_ptr_s, &Rf->above_mass_avg.phi);
    CHKERRQ(ierr);
    ierr = average_by_mass_staggered(E, M->radius_s, mask_ptr_s, &Rf->above_mass_avg.depth);
    CHKERRQ(ierr);
    ...
    ierr = VecGetValues(S->phi, 1, &index_below, &phi);
    CHKERRQ(ierr);
    Rf->below_middle.phi = phi;
    ierr = VecGetValues(M->radius_b, 1, &index_below, &radius);
    CHKERRQ(ierr);
    ...
    /* average by mass */
    ierr = invert_vec_mask(mask_ptr_s);
    CHKERRQ(ierr);
    ierr = average_by_mass_staggered(E, S->phi_s, mask_ptr_s, &Rf->below_mass_avg.phi);
    CHKERRQ(ierr);
    ierr = average_by_mass_staggered(E, M->radius_s, mask_ptr_s, &Rf->below_mass_avg.depth);
    CHKERRQ(ierr);
