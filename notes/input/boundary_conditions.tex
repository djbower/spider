...
\end{equation}
Particularly due to the functional dependences of $\kappa_h$, it could be quite nasty to take the time derivative.  So instead assume that $\kappa_h$ and $\rho$ can be approximated from the basic node below the surface (subscript 1).  \dbnote{TODO: in reality, these can probably be any appropriately chosen constant values based on the current state.}
\begin{equation}
...
\end{equation}
The above couples the updates for the top staggered node $S_{1/2}$ and the surface entropy gradient.  We can similarly simplify the time derivative of the conductive heat contribution to also include on the RHS as a contribution to the geothermal flux \dbnote{TODO}.
