...
\end{equation}
For several reactions that we consider, the $D$ product is actually oxygen, which is constrained by the evolution of the interior.  Hence we have expressions that give $fO_2$ as a function of temperature $T$.  \textbf{Note that $fO_2$ here is the volume mixing ratio, already normalised by the total (surface) pressure!}. For the form given in \cite{SF17} (and yes, same symbols for constants are used as the stoichiometry above, but in fact they are different!):
\begin{equation}
\log_{10} fO_2 = a + b \frac{10^3}{T} + c \frac{10^6}{T^2} + d \frac{10^9}{T^3} + f \frac{10^{12}}{T^4} = g(T)
\end{equation}
\begin{equation}
fO_2 = 10^{g(T)}
\end{equation}
...
Where:
\begin{equation}
\frac{dg}{dT} = -b \frac{10^3}{T^2} - 2c \frac{10^6}{T^3} - 3d \frac{10^9}{T^4} - 4f \frac{10^{12}}{T^5}
\end{equation}
\dbnote{TODO: an alternative but similar form for $fO_2$ is given in Olson and Sharp (2019), and the derivative can be determined similarly.}.  The denominator of the reaction quotient is:
\begin{equation}
Q_r = A^{-a} B^{-b} P_s^{-f}
\end{equation}
