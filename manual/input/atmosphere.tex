...
\begin{equation}
X_v = V_0 \hat{X}_v
% \frac{X_{\rm ppm}}{V_0} = \frac{10^6 X_v}{V_0}
\end{equation}
...
\boxedeq{}{V_0 \hat{X}_v (k_v \hat{M}^s + \hat{M}^l) + \frac{\hat{R}_p^2}{\hat{g}} \left( \frac{\hat{\mu}_v^g}{\hat{\bar{\mu}}} \right) \hat{p}_v + \frac{1}{M_0} \left( m_v^e + m_v^o + m_v^r \right) = V_0 \hat{X}_v^{\rm init} \hat{M}^m \label{eq:volevo}}
%%%%
