...
We use a chemical model similar to \citep{GS14}, wherein we assume chemical reactions are at equilibrium at each time step and then we calculate the change in concentration of reactants and products based off the how much of any volatile is added to the system. For the reaction that produces water:
\begin{equation}
...
\end{equation}
Which can also be rewritten in terms of the oxygen fugacity, $fO_2$, as: 
\begin{equation}
    \frac{[H_2O]}{[H_2]} = K_{eq}fO_2^{1/2}
\end{equation}
The equilibrium constant for this reaction can be calculated based off the surface temperature using data from \citep{RBF78}, and a fit from \citep{OS19}: 
\begin{equation}
    log_{10}K_{eq} = 7.39\times 10^5T_s^{-1.61}.
\end{equation}
The oxygen fugacity, which is determined by the iron-wustite buffer, can be calculated using the interior pressure and temperature using either: 
\begin{equation}
...
\end{equation}
from \citep{F91}. Neither of these are currently implemented. Instead, I have averaged over the temperature domain and set $K_{eq}fO_2^{1/2} = 0.01$, for simplicity. 
If the reaction is not in equilibrium, we can use the same expression to calculate the reaction quotient, Q. We can compare Q to K to tell whether the system is in equilibrium and if it is not, how it will shift to go back into equilibrium. If the reaction quotient is not equal to the equilibrium constant, the concentrations of products will change by some amount, $x$: 
...
\end{equation}
and we will calculate the difference in concentration using: 
\begin{equation}
K = 
     \begin{dcases}
...
The relationship between the concentration of a given volatile and it's mass concentration evolution is: 
\begin{equation}
    \frac{dm_{\rm v}^r}{dt} = sn_{\rm v}C_{\rm v}x
\end{equation}
