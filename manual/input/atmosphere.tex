...
\end{equation}
For the $B$ term we have to account for $G$, the modified equilibrium constant:
\begin{equation}
...
\end{equation}
Now let's take a look at the derivative of $G$.  In our formulation $G(T)$ (since $f_{O_2}$ is a function of $T$ and so is the equilibrium constant).  Therefore we can compute the derivative with respect to $T$ quite easily:
\begin{equation}
\frac{dG}{d \hat{p}_v} = \frac{dG}{d \hat{T}} \frac{d \hat{T}}{d \hat{p}_v} = \frac{dG}{d \hat{T}} \frac{d \hat{T}}{dt} \frac{dt}{d\hat{p}_v}
\end{equation}
The expansion for the derivative with respect to $\hat{p}_v$ is done in terms of $t$, since both of those derivatives can be computed using the code.
