...
Therefore the mass balance in non-dimensional form is, \myemph{where again, masses are scaled masses without the $4 \pi$ term}:  \dbnote{Note that here the terms such as escape $m_v^e$ are also now omitting the $4 \pi$ factor}
...
\end{align}
Since the final term on the LHS has not been defined, we keep the terms dimensional inside the brackets.  \dbnote{Note that here the terms such as escape $m_v^e$ are still omitting the $4 \pi$ factor}
...
We can also express the evolution equation using partial pressure as the primary (solution) variable, in which case:
%%%
%%% PRIMARY PV HERE
%%%
\begin{align}
&\frac{d \hat{X}_v^{\rm l}}{d\hat{p}_v} \frac{d \hat{p}_v}{d\hat{t}}
%&\frac{\hat{\alpha}_v}{\hat{\beta}_v} \left( \frac{\hat{X}_v}{\hat{\alpha}_v} \right)^{1-\beta_v}
\left(k_{\rm v} \hat{M}^{\rm m} + (1-k_{\rm v}) \hat{M}^{\rm l} \right)
+ \hat{X}_v^{\rm l} (1-k_{\rm v}) \frac{d \hat{M}^{\rm l}}{d\hat{t}} + \nonumber \\
&\quad \frac{10^6}{V_0} \frac{\hat{R}_{\rm p}^2 \hat{\mu}_{\rm v}^{\rm g}}{\hat{g}}
\left[
\frac{1}{\hat{\mu}_{\rm t}} \left( \frac{d \hat{p}_v}{d \hat{t}} \right)
-\frac{\hat{p_v}}{\hat{\mu}_{\rm t}^2} \sum_i^n \hat{\mu}_i \left[ \frac{1}{P_s} \frac{\partial \hat{p}_i}{\partial \hat{t}} - \frac{\hat{p}_i}{P_s} \frac{1}{P_s} \frac{\partial P_s}{\partial \hat{t}} \right]
\right]+ \nonumber \\
& \qquad \frac{10^6}{V_0} \frac{\tau_0}{M_0} \left( \frac{dm_v^e}{dt} +  \frac{dm_v^o}{dt} + \frac{dm_v^r}{dt} \right) = 0
\end{align}
Basically, any prescribed relationship between $X^l_v$ and $p_v$ can be used in this equation, whether it's Henry's law or a more sophisticated formulation.  This offers opportunity to investigate ``fugacity crisis'' scenarios \citep{KFS19,KFS20}
%%%%
