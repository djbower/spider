...
\end{equation}
where $N_A$ is Advogadro's number.  See the aforementioned paper for details, but $S$ is non-dimensional and $f_{H_2}$ is non-dimensional, and $\Gamma$ is a non-dimensional constant I introduced to scale the whole expression.  Hence all the dimensionality is wrapped up in the constant.  Therefore the escape rate of mass (using the definition of $\phi_{H_2}$ with units of moles), is:
\begin{equation}
\frac{dm_{\rm v}^{\rm esc}}{dt} = 4\pi R_p^2 \phi_{H_2} \mu_{H_2}
\end{equation}
where $\mu_{H_2}$ is the molar mass of H$_2$.  Now non-dimensionalise the constant:
\begin{equation}
\frac{10^{12} \cdot 10^{4}}{N_A} \qquad \rm{moles}\ {\rm m}^{-2}\ {\rm s}^{-1}
\end{equation}
Using the non-dimensional scheme of SPIDER:
\begin{equation}
\hat{\phi}_{H_2} = \frac{10^{12} \cdot 10^{4}}{N_A} \times R_0^2 \cdot \tau_0 \qquad \rm{moles}
\end{equation}
Recall that non-dimensional $\mu$ in SPIDER has units of mol$^{-1}$ which will cancel with the moles above.  Hence the non-dimensional escape rate is:
\begin{equation}
\frac{d\hat{m}_{\rm v}^{\rm esc}}{d\hat{t}} = 4\pi \hat{R}_p^2 \hat{\phi}_{H_2} \hat{\mu}_{H_2}
\end{equation}
Still need the extra factors associated with SPIDER:
\begin{equation}
\frac{d\hat{m}_{\rm v}^{\rm esc}}{d\hat{t}} = \frac{10^6}{V_0} \frac{1}{4\pi} 4\pi \hat{R}_p^2 \hat{\phi}_{H_2} \hat{\mu}_{H_2}
\end{equation}
