...
\end{equation}
Now, $dS_{-1}/dt$ is the change in entropy of the last staggered node, which is given by the ``master'' equation since it depends on fluxes in and out of the volume as well as interior heat sources.  $E_{cmb}$ can be computed based on the heat flux at the CMB in the mantle or set to a fixed value.  If $E_{cmb}=0$ then an isothermal boundary condition is naturally recovered.  Hence this one equation is now implemented in SPIDER.
%%%
