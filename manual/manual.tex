...
\end{equation}
Note that $\xi$ is referred to by \cite{ABE95} as a ``mass coordinate'' and has \textbf{length dimension}.  Due to the choice of integration limits, it's clear that $r=r_{\rm cmb}$ when $\xi=0$.  To implement, we actually define the mesh using radial coordinates, and then map to mass coordinates using Eq.~\ref{eq:masscoord}.  This is because the Adams-Williamson equation of state gives a simple form of the density and the integral involving $r$ on the RHS of Eq.~\ref{eq:masscoord} can be evaluated directly.  Furthermore, we are free to define $\rho_0$ as we wish, since it is a perfectly arbitrary constant.  Nevertheless, it is intuitive to set $\rho_0$ such that $\xi=r$ at the planetary surface.  Therefore, the bounds of radius are $r_{\rm cmb}$ to $r$, and the bounds of the mass coordinate $\xi$ are $0$ to $r$.
