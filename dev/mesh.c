...
static PetscErrorCode SetMeshGeometricRefineUpper(Ctx *E)
{
    ...
    CHKERRQ(ierr);
    ihi_b = ilo_b + w_b;
    ierr = DMDAVecGetArray(da_b, M->xi_b, &arr_b);
    CHKERRQ(ierr);
    arr_b[0] = P->radius;
    ...
    ierr = DMDAVecRestoreArray(da_b, M->xi_b, &arr_b);
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ihi_s = ilo_s + w_s;
    ierr = DMDAVecGetArray(da_s, M->xi_s, &arr_s);
    CHKERRQ(ierr);
    for (i = ilo_s; i < ihi_s; ++i)
    {
        arr_s[i] = 0.5 * (arr_b[i] + arr_b[i + 1]);
    }
    ierr = DMDAVecRestoreArray(da_s, M->xi_s, &arr_s);
    CHKERRQ(ierr);
