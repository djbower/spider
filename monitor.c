...
PetscErrorCode TSCustomMonitor(TS ts, PetscInt step, PetscReal time, Vec sol, void *ptr, MonitorCtx *mctx)
{
  ...
  /* Info for stdout */
  {
    ...
    ierr = PetscMalloc4(ctx->numFields, &fieldActive, ctx->numFields, &subVecs, ctx->numFields, &maxVals, ctx->numFields, &minVals);
    CHKERRQ(ierr);
    ierr = DMCompositeGetAccessArray(ctx->dm_sol, sol, ctx->numFields, NULL, subVecs);
    CHKERRQ(ierr);

    for (i = 0; i < ctx->numFields; ++i)
    {
      ierr = VecGetSize(subVecs[i], &len);
      CHKERRQ(ierr);
      fieldActive[i] = len > 0;
      if (fieldActive[i])
      {
        ierr = VecMin(subVecs[i], NULL, &minVals[i]);
        CHKERRQ(ierr);
        ierr = VecMax(subVecs[i], NULL, &maxVals[i]);
        CHKERRQ(ierr);
      }
    }
    ierr = DMCompositeRestoreAccessArray(ctx->dm_sol, sol, ctx->numFields, NULL, subVecs);
    CHKERRQ(ierr);
    ...
    /* Cleanup */
    ierr = PetscFree4(fieldActive, subVecs, maxVals, minVals);
    CHKERRQ(ierr);
  }
  ...
  {
    PetscBool exists;
    ierr = PetscTestDirectory(P->outputDirectory, 'w', &exists);
    CHKERRQ(ierr);
    if (!exists)
    {
      ierr = PetscMkdir(P->outputDirectory);
      CHKERRQ(ierr);
    }
  ...
  /* Dump a JSON file for this timestep */
  {
    ...
    if (rank == 0)
    {
      ...
      ierr = PetscSNPrintf(filename, PETSC_MAX_PATH_LEN, "%s/%lld.json", P->outputDirectory, time_years_actual);
      CHKERRQ(ierr);
      ...
      {
        cJSON *solJSON;
        ierr = DimensionalisableFieldToJSON(ctx->solDF, &solJSON);
        CHKERRQ(ierr);
      ...
      /* Re-evaluate rhs and add to file */
      {
        Vec rhs;
        cJSON *rhsJSON;
        ierr = DimensionalisableFieldGetGlobalVec(ctx->rhsDF, &rhs);
        CHKERRQ(ierr);
        ierr = RHSFunction(ts, time, sol, rhs, ctx);
        CHKERRQ(ierr);
        ierr = DimensionalisableFieldToJSON(ctx->rhsDF, &rhsJSON);
        CHKERRQ(ierr);
        cJSON_AddItemToObject(json, "rhs", rhsJSON);
        CHKERRQ(ierr);
      }
      ...
      // Note: this is duplicative, but it's too painful to flatten out the Ctx
      {
        ...
        if (ctx->parameters->n_phases > 1)
        {
          ...
          ierr = EOSCompositeGetSubEOS(ctx->parameters->eos, &sub_eos, &should_be_two);
          CHKERRQ(ierr);
          if (should_be_two != 2)
            SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_SUP, "Expecting two sub-EOSs");
          ierr = JSON_add_phase_boundary(ctx, sub_eos[0], "liquidus", data);
          CHKERRQ(ierr);
          ierr = JSON_add_phase_boundary(ctx, sub_eos[1], "solidus", data);
          CHKERRQ(ierr);
        }
      ...
      ierr = JSON_add_rheological_front(ctx->da_point, ctx->parameters->scaling_constants, &ctx->rheological_front_phi, "rheological_front_phi", json);
      CHKERRQ(ierr);

      /* rheological front dynamic */
      ierr = JSON_add_rheological_front(ctx->da_point, ctx->parameters->scaling_constants, &ctx->rheological_front_dynamic, "rheological_front_dynamic", json);
      CHKERRQ(ierr);

      /* atmosphere */
      ierr = JSON_add_atmosphere(ctx->da_point, ctx->parameters, &ctx->atmosphere, "atmosphere", json);
      CHKERRQ(ierr);
...
PetscErrorCode SNESMonitorVerbose(SNES snes, PetscInt its, PetscReal norm, void *mctx)
{
  ...
  CHKERRQ(ierr);
  ierr = SNESGetFunction(snes, &r, &func, &ctx);
  CHKERRQ(ierr); /* ctx should be the same as mctx */
