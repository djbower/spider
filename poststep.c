...
PetscErrorCode PostStep(TS ts)
{
  ...
  /* save current volatile concentrations in the melt */
  i = P->numpts_b + 1;
  ierr = VecGetValues(X,1,&i,&x_CO2);CHKERRQ(ierr);
  i = P->numpts_b + 2;
  ierr = VecGetValues(X,1,&i,&x_H2O);CHKERRQ(ierr);

  //if ((data->x_CO2 > 1.1 * Ap->CO2_parameters.initial) || (data->x_H2O > 1.1* Ap->H2O_parameters.initial)){
  if ((x_CO2 > 1.1*data->x_CO2) || (x_H2O > 1.1*data->x_H2O)){
