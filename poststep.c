...
PetscErrorCode PostStepDataInitialize(Ctx *E)
{
  ...
  ierr = PetscMalloc(sizeof(PostStepData), &(E->postStepData));
  CHKERRQ(ierr);
...
PetscErrorCode PostStep(TS ts)
{
  ...
  PetscFunctionBeginUser;
  ierr = TSGetStepNumber(ts, &stepNumber);
  CHKERRQ(ierr);
  ierr = TSGetApplicationContext(ts, &E);
  CHKERRQ(ierr);
  A = &E->atmosphere;
  P = E->parameters;
  Ap = P->atmosphere_parameters;
  if (!E->parameters->rollBackActive)
    SETERRQ(PetscObjectComm((PetscObject)ts), PETSC_ERR_SUP, "You must run with -activate_rollback");
  data = (PostStepData *)E->postStepData;
