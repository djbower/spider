...
PetscErrorCode JSON_add_phase_boundary(const Ctx *E, const EOS Ep, const char *name, cJSON *json)
{
    ...
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldSetUnits(DF_phase_b, "J kg$^{-1}$ K$^{-1}$");
    CHKERRQ(ierr);

    /* phase boundary defined by temperature */
    strcat(strcpy(namestr2, name), "_temp_b"); // phase boundary defined by temperature
    scaling = SC->TEMP;
    ierr = DimensionalisableFieldCreate(&DF_phase_temp_b, da_b, &scaling, PETSC_FALSE);
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldGetGlobalVec(DF_phase_temp_b, &phase_temp_b);
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldSetName(DF_phase_temp_b, namestr2);
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldSetUnits(DF_phase_temp_b, "K");
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ierr = DMDAVecGetArray(da_b, phase_temp_b, &arr_phase_temp_b);
    CHKERRQ(ierr);

    for (i = ilo_b; i < ihi_b; ++i)
    {
        ierr = EOSGetPhaseBoundary(Ep, arr_pres_b[i], &Sbound, NULL);
        CHKERRQ(ierr);
        arr_phase_b[i] = Sbound;
        ierr = EOSEval(Ep, arr_pres_b[i], Sbound, &eos_eval);
        CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArray(da_b, phase_temp_b, &arr_phase_temp_b);
    CHKERRQ(ierr);
    ...
    ierr = DimensionalisableFieldToJSON(DF_phase_temp_b, &data);
    CHKERRQ(ierr);
    cJSON_AddItemToObject(json, namestr2, data);

    ierr = DimensionalisableFieldDestroy(&DF_phase_b);
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldDestroy(&DF_phase_temp_b);
    CHKERRQ(ierr);
