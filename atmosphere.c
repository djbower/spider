...
PetscErrorCode initialise_atmosphere(Atmosphere *A, const AtmosphereParameters Ap, const ScalingConstants SC)
{
    ...
    ierr = DMDACreate1d(PETSC_COMM_WORLD, DM_BOUNDARY_NONE, numpts, dof, stencilWidth, NULL, &A->da_atm);
    CHKERRQ(ierr);
    ierr = DMSetUp(A->da_atm);
    CHKERRQ(ierr);
    ...
    scaling = 1.0;
    ierr = DimensionalisableFieldCreate(&A->atm_struct[0], A->da_atm, &scaling, PETSC_FALSE);
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldGetGlobalVec(A->atm_struct[0], &A->atm_struct_tau);
    ierr = DimensionalisableFieldSetName(A->atm_struct[0], "atm_struct_tau");
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldSetUnits(A->atm_struct[0], "None");
    CHKERRQ(ierr);

    scaling = SC->TEMP;
    ierr = DimensionalisableFieldCreate(&A->atm_struct[1], A->da_atm, &scaling, PETSC_FALSE);
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldGetGlobalVec(A->atm_struct[1], &A->atm_struct_temp);
    ierr = DimensionalisableFieldSetName(A->atm_struct[1], "atm_struct_temp");
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldSetUnits(A->atm_struct[1], "K");
    CHKERRQ(ierr);

    scaling = SC->PRESSURE / 1.0E5; // bar
    ierr = DimensionalisableFieldCreate(&A->atm_struct[2], A->da_atm, &scaling, PETSC_FALSE);
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldGetGlobalVec(A->atm_struct[2], &A->atm_struct_pressure);
    ierr = DimensionalisableFieldSetName(A->atm_struct[2], "atm_struct_pressure");
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldSetUnits(A->atm_struct[2], "bar");
    CHKERRQ(ierr);

    scaling = SC->RADIUS;
    ierr = DimensionalisableFieldCreate(&A->atm_struct[3], A->da_atm, &scaling, PETSC_FALSE);
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldGetGlobalVec(A->atm_struct[3], &A->atm_struct_depth);
    ierr = DimensionalisableFieldSetName(A->atm_struct[3], "atm_struct_depth");
    CHKERRQ(ierr);
    ierr = DimensionalisableFieldSetUnits(A->atm_struct[3], "m");
    CHKERRQ(ierr);
...
PetscErrorCode destroy_atmosphere(Atmosphere *A)
{
    ...
    {
        ierr = DimensionalisableFieldDestroy(&A->atm_struct[i]);
        CHKERRQ(ierr);
    }

    ierr = DMDestroy(&A->da_atm);
    CHKERRQ(ierr);
...
static PetscErrorCode set_atm_struct_tau(Atmosphere *A)
{
    ...
    ierr = DMDAGetInfo(A->da_atm, NULL, &numpts, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
    CHKERRQ(ierr);

    ierr = DMDAGetCorners(A->da_atm, &ilo, 0, 0, &w, 0, 0);
    CHKERRQ(ierr);
    ihi = ilo + w;
    ...
    for (i = ilo; i < ihi; ++i)
    {
        ...
        ierr = VecSetValues(A->atm_struct_tau, 1, &i, &tau, INSERT_VALUES);
        CHKERRQ(ierr);
    }

    ierr = VecAssemblyBegin(A->atm_struct_tau);
    CHKERRQ(ierr);
    ierr = VecAssemblyEnd(A->atm_struct_tau);
    CHKERRQ(ierr);
...
static PetscErrorCode set_atm_struct_temp(Atmosphere *A, const AtmosphereParameters Ap, const FundamentalConstants FC)
{
    ...
    ierr = VecCopy(A->atm_struct_tau, A->atm_struct_temp);
    CHKERRQ(ierr);
    ierr = VecShift(A->atm_struct_temp, 1.0);
    CHKERRQ(ierr);
    ierr = VecScale(A->atm_struct_temp, 0.5);
    CHKERRQ(ierr);
    ierr = VecScale(A->atm_struct_temp, TO4);
    CHKERRQ(ierr);
    ierr = VecShift(A->atm_struct_temp, Teq4);
    CHKERRQ(ierr);
    ierr = VecPow(A->atm_struct_temp, 1.0 / 4.0);
    CHKERRQ(ierr);
...
static PetscErrorCode set_atm_struct_pressure(Atmosphere *A, const AtmosphereParameters Ap)
{
    ...
    ierr = VecCopy(A->atm_struct_tau, A->atm_struct_pressure);
    CHKERRQ(ierr);
    ierr = VecScale(A->atm_struct_pressure, -(*Ap->gravity_ptr));
    CHKERRQ(ierr); // note negative gravity
    ierr = VecScale(A->atm_struct_pressure, 2.0);
    CHKERRQ(ierr);
    ierr = VecScale(A->atm_struct_pressure, 1.0 / 3.0);
    CHKERRQ(ierr);
    ierr = VecScale(A->atm_struct_pressure, 1.0 / kabs_tot);
    CHKERRQ(ierr);
...
PetscErrorCode set_reservoir_volatile_content(Atmosphere *A, const AtmosphereParameters Ap, const FundamentalConstants FC, const ScalingConstants SC)
{
    ...
    ierr = set_total_surface_pressure(A, Ap);
    CHKERRQ(ierr);

    ierr = set_volatile_abundances_from_partial_pressure(A, Ap, SC);
    CHKERRQ(ierr);

    ierr = set_volume_mixing_ratios(A, Ap);
    CHKERRQ(ierr);

    ierr = set_volatile_masses_in_atmosphere(A, Ap);
    CHKERRQ(ierr);

    ierr = set_volatile_masses_in_liquid(A, Ap);
    CHKERRQ(ierr);

    ierr = set_volatile_masses_in_solid(A, Ap);
    CHKERRQ(ierr);

    ierr = set_volatile_masses_reactions(A, Ap);
    CHKERRQ(ierr);
    ...
    ierr = set_jeans_escape(A, Ap, FC);
    CHKERRQ(ierr);

    ierr = set_zahnle_escape(A, Ap);
    CHKERRQ(ierr);
...
static PetscErrorCode set_jeans_escape(Atmosphere *A, const AtmosphereParameters Ap, const FundamentalConstants FC)
{
    ...
    for (i = 0; i < Ap->n_volatiles; ++i)
    {
        ...
        ierr = set_column_density_volatile(A, Ap, FC, i);
        CHKERRQ(ierr);

        ierr = set_jeans(A, Ap, FC, i);
        CHKERRQ(ierr);

        ierr = set_Knudsen_number(A, Ap, i);
        CHKERRQ(ierr);

        ierr = set_R_thermal_escape(A, Ap, i);
        CHKERRQ(ierr);

        ierr = set_f_thermal_escape(A, i);
        CHKERRQ(ierr);
    }
...
static PetscErrorCode set_zahnle_escape(Atmosphere *A, const AtmosphereParameters Ap)
{
    ...
    for (i = 0; i < Ap->n_volatiles; ++i)
    {
        ierr = set_f_zahnle_escape(A, Ap, i);
        CHKERRQ(ierr);
    }
...
PetscErrorCode set_atmosphere_emissivity_and_flux(Atmosphere *A, const AtmosphereParameters Ap, const FundamentalConstants FC, const ScalingConstants SC)
{
    ...
    case 5:
        /* isothermal */
        /* nothing to do, since bcs constrain the surface temperature
...
static PetscErrorCode JSON_add_atm_struct(Atmosphere *A, const AtmosphereParameters Ap, const FundamentalConstants FC, cJSON *data)
{
    ...
    ierr = set_atm_struct_tau(A);
    CHKERRQ(ierr);
    ierr = set_atm_struct_temp(A, Ap, FC);
    CHKERRQ(ierr);
    ierr = set_atm_struct_pressure(A, Ap);
    CHKERRQ(ierr);
    ierr = set_atm_struct_depth(A, Ap, FC);
    CHKERRQ(ierr);
    ...
    for (i = 0; i < NUMATMSTRUCTVECS; ++i)
    {
        ...
        ierr = DimensionalisableFieldToJSON(curr, &item);
        CHKERRQ(ierr);
...
PetscErrorCode JSON_add_atmosphere(DM dm, Parameters const P, Atmosphere *A, const char *name, cJSON *json)
{
    ...
    if (Ap->SURFACE_BC == 3)
    {
        ...
        ierr = JSON_add_atm_struct(A, Ap, FC, data);
        CHKERRQ(ierr);
        /* optical depth, non-dimensional */
        scaling = 1.0;
        ierr = JSON_add_single_value_to_object(dm, scaling, "optical_depth", "None", A->tau, data);
        CHKERRQ(ierr);
    }
    ...
    if (Ap->SURFACE_BC != 5)
    {
        ...
        scaling = 1.0;
        ierr = JSON_add_single_value_to_object(dm, scaling, "emissivity", "None", A->emissivity, data);
        CHKERRQ(ierr);
        /* net upward atmospheric flux */
        scaling = SC->FLUX;
        ierr = JSON_add_single_value_to_object(dm, scaling, "Fatm", "W m$^{-2}$", A->Fatm, data);
        CHKERRQ(ierr);
        ...
        scaling = SC->TEMP;
        ierr = JSON_add_single_value_to_object(dm, scaling, "Teff", "K", Teff, data);
        CHKERRQ(ierr);
        Tskin = A->Fatm / (2 * FC->STEFAN_BOLTZMANN) + PetscPowScalar(Ap->teqm, 4.0);
        Tskin = PetscPowScalar(Tskin, 1.0 / 4.0);
        ierr = JSON_add_single_value_to_object(dm, scaling, "Tskin", "K", Tskin, data);
        CHKERRQ(ierr);
    }

    /* total liquid mass of mantle, kg */
    scaling = 4.0 * PETSC_PI * SC->MASS; // includes 4*PI for spherical geometry
    ierr = JSON_add_single_value_to_object(dm, scaling, "mass_liquid", "kg", A->Mliq, data);
    CHKERRQ(ierr);
    /* total solid mass of mantle, kg */
    ierr = JSON_add_single_value_to_object(dm, scaling, "mass_solid", "kg", A->Msol, data);
    CHKERRQ(ierr);
    /* total mass of mantle, kg (for sanity check) */
    ierr = JSON_add_single_value_to_object(dm, scaling, "mass_mantle", "kg", *Ap->mantle_mass_ptr, data);
    CHKERRQ(ierr);
    /* total mass of core, kg */
    ierr = JSON_add_single_value_to_object(dm, scaling, "mass_core", "kg", P->coremass, data);
    CHKERRQ(ierr);

    /* kg, without 4*pi */
    val = SC->MASS * A->molar_mass * 1.0E3;
    ierr = JSON_add_single_value_to_object(dm, 1.0, "molar_mass", "g/mol", val, data);
    CHKERRQ(ierr);

    /* surface temperature, K */
    scaling = SC->TEMP;
    ierr = JSON_add_single_value_to_object(dm, scaling, "temperature_surface", "K", A->tsurf, data);
    CHKERRQ(ierr);

    /* surface pressure, bar */
    scaling = SC->PRESSURE / 1.0E5; /* bar */
    ierr = JSON_add_single_value_to_object(dm, scaling, "pressure_surface", "bar", A->psurf, data);
    CHKERRQ(ierr);
    ...
    if (Ap->OXYGEN_FUGACITY)
    {
        ...
        scaling = 1.0;
        ierr = JSON_add_single_value_to_object(dm, scaling, "fO2", "bar", A->fO2, data);
        CHKERRQ(ierr);
    }
    ...
    {
        ierr = JSON_add_volatile(dm, P, Ap->volatile_parameters[v], &A->volatiles[v], A, Ap->volatile_parameters[v]->prefix, data);
        CHKERRQ(ierr);
    }
...
static PetscErrorCode JSON_add_volatile(DM dm, Parameters const P, VolatileParameters const VP, Volatile const *V, Atmosphere const *A, char const *name, cJSON *json)
{
    ...
    ierr = JSON_add_single_value_to_object(dm, scaling, "initial_ppmw", "ppmw", VP->initial_total_abundance, data);
    CHKERRQ(ierr);
    /* volatile in liquid mantle (ppmw) */
    ierr = JSON_add_single_value_to_object(dm, scaling, "liquid_ppmw", "ppmw", V->x, data);
    CHKERRQ(ierr);
    /* volatile in solid mantle (ppmw) */
    ierr = JSON_add_single_value_to_object(dm, scaling, "solid_ppmw", "ppmw", V->x * VP->kdist, data);
    CHKERRQ(ierr);

    /* ocean moles */
    scaling = 1.0;
    ierr = JSON_add_single_value_to_object(dm, scaling, "initial_ocean_moles", "ocean moles", VP->initial_ocean_moles, data);
    CHKERRQ(ierr);

    /* kilograms (kg) */
    scaling = SC->VOLATILE * 4.0 * PETSC_PI * SC->MASS;
    /* initial volatile (kg) */
    ierr = JSON_add_single_value_to_object(dm, scaling, "initial_kg", "kg", VP->initial_total_abundance * (*Ap->mantle_mass_ptr), data);
    CHKERRQ(ierr);
    /* volatile in liquid mantle (kg) */
    ierr = JSON_add_single_value_to_object(dm, scaling, "liquid_kg", "kg", V->mass_liquid, data);
    CHKERRQ(ierr);
    /* volatile in solid mantle (kg) */
    ierr = JSON_add_single_value_to_object(dm, scaling, "solid_kg", "kg", V->mass_solid, data);
    CHKERRQ(ierr);
    /* volatile in atmosphere (kg) */
    ierr = JSON_add_single_value_to_object(dm, scaling, "atmosphere_kg", "kg", V->mass_atmos, data);
    CHKERRQ(ierr);
    /* volatile exchanges due to reactions (kg) */
    ierr = JSON_add_single_value_to_object(dm, scaling, "reaction_kg", "kg", V->mass_reaction, data);
    CHKERRQ(ierr);
    /* physical reservoir size */
    val = V->mass_liquid + V->mass_solid + V->mass_atmos;
    ierr = JSON_add_single_value_to_object(dm, scaling, "physical_kg", "kg", val, data);
    CHKERRQ(ierr);
    ...
    scaling = SC->PRESSURE / 1.0E5; /* bar */
    ierr = JSON_add_single_value_to_object(dm, scaling, "atmosphere_bar", "bar", V->p, data);
    CHKERRQ(ierr);
    scaling = SC->PRESSURE / 1.0E5 / SC->TIMEYRS; /* bar / year */
    ierr = JSON_add_single_value_to_object(dm, scaling, "dpdt", "bar/yr", V->dpdt, data);
    CHKERRQ(ierr);

    /* area */
    scaling = 1.0 / (SC->AREA * 1.0E4); // 1/cm^2
    ierr = JSON_add_single_value_to_object(dm, scaling, "column_density", "1/cm^2", V->column_density, data);
    CHKERRQ(ierr);

    /* non-dimensional */
    scaling = 1.0;
    ierr = JSON_add_single_value_to_object(dm, scaling, "optical_depth", "None", V->tau, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, scaling, "mixing_ratio", "None", V->mixing_ratio, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, scaling, "jeans", "None", V->jeans, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, scaling, "Knudsen", "None", V->Knudsen, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, scaling, "R_thermal_escape", "None", V->R_thermal_escape, data);
    CHKERRQ(ierr);
    ierr = JSON_add_single_value_to_object(dm, scaling, "f_thermal_escape", "None", V->f_thermal_escape, data);
    CHKERRQ(ierr);
...
PetscScalar get_dpdt(Atmosphere *A, const AtmosphereParameters Ap, PetscInt i, const PetscScalar *dmrdt, const ScalingConstants SC)
{
    ...
    ierr = set_total_surface_pressure_time_derivative(A, Ap);
    CHKERRQ(ierr);
...
static PetscErrorCode set_atm_struct_depth(Atmosphere *A, const AtmosphereParameters Ap, const FundamentalConstants FC)
{
    ...
    ierr = DMDAVecGetArrayRead(A->da_atm, A->atm_struct_tau, &arr_tau);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArray(A->da_atm, A->atm_struct_depth, &arr_depth);
    CHKERRQ(ierr);
    ...
    ierr = DMDAVecRestoreArrayRead(A->da_atm, A->atm_struct_tau, &arr_tau);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArray(A->da_atm, A->atm_struct_depth, &arr_depth);
    CHKERRQ(ierr);
