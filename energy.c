...
static PetscErrorCode append_Hradio(Ctx *E, PetscReal time)
{
    ...
    for (i = 0; i < P->n_radionuclides; ++i)
    {
        ...
        ierr = VecShift(S->Hradio_s, H);
        CHKERRQ(ierr);
    }

    // append total of radiogenic heating to total heating vector
    ierr = VecAXPY(S->Htot_s, 1.0, S->Hradio_s);
    CHKERRQ(ierr);
...
PetscErrorCode set_Etot(Ctx *E)
{
    ...
    ierr = set_Jtot(E);
    CHKERRQ(ierr);
    ierr = VecPointwiseMult(S->Etot, S->Jtot, M->area_b);
    CHKERRQ(ierr);
...
static PetscErrorCode append_Jconv(Ctx *E)
{
    ...
    for (i = ilo_b; i < ihi_b; ++i)
    {
        ...
        ierr = VecSetValue(S->Jconv, i, Jconv, INSERT_VALUES);
        CHKERRQ(ierr);
    }

    ierr = VecAssemblyBegin(S->Jconv);
    CHKERRQ(ierr);
    ierr = VecAssemblyEnd(S->Jconv);
    CHKERRQ(ierr);

    ierr = VecAXPY(S->Jtot, 1.0, S->Jconv);
    CHKERRQ(ierr);
...
PetscScalar GetConvectiveHeatFlux(Ctx *E, PetscInt *ind_ptr)
{
    ...
    if (!*ind_ptr)
    {
        ierr = VecGetValues(S->kappah, 1, &ind1, &kappah);
        CHKERRQ(ierr);
    }
    ...
    else if (*ind_ptr == ind_cmb)
    {
        /* use zero */
        // kappah = 0.0;
        ierr = VecGetValues(S->kappah, 1, &ind_abv_cmb, &kappah);
        CHKERRQ(ierr);
    }
    /* otherwise use kappah at this node */
    else
    {
        ierr = VecGetValues(S->kappah, 1, ind_ptr, &kappah);
        CHKERRQ(ierr);
    }
...
static PetscErrorCode append_Jmix(Ctx *E)
{
    ...
    for (i = ilo_b; i < ihi_b; ++i)
    {
        ...
        ierr = VecSetValue(S->Jmix, i, Jmix, INSERT_VALUES);
        CHKERRQ(ierr);
    }

    ierr = VecAssemblyBegin(S->Jmix);
    CHKERRQ(ierr);
    ierr = VecAssemblyEnd(S->Jmix);
    CHKERRQ(ierr);

    ierr = VecAXPY(S->Jtot, 1.0, S->Jmix);
    CHKERRQ(ierr);
...
PetscScalar GetMixingHeatFlux(Ctx *E, PetscInt *ind_ptr)
{
    ...
    ierr = EOSCompositeGetSubEOS(P->eos, &sub_eos, &should_be_two);
    CHKERRQ(ierr);
    if (should_be_two != 2)
        SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_SUP, "Expecting two sub-EOSs");
    ...
    ierr = VecGetValues(M->dxidr_b, 1, ind_ptr, &dxidr);
    CHKERRQ(ierr);
    ierr = VecGetValues(M->pressure_b, 1, ind_ptr, &pres);
    CHKERRQ(ierr);
    ierr = VecGetValues(S->rho, 1, ind_ptr, &rho);
    CHKERRQ(ierr);
    ierr = VecGetValues(S->kappac, 1, ind_ptr, &kappac);
    CHKERRQ(ierr);
    ierr = VecGetValues(S->phi, 1, ind_ptr, &phi);
    CHKERRQ(ierr);
...
static PetscErrorCode append_Jcond(Ctx *E)
{
    ...
    for (i = ilo_b; i < ihi_b; ++i)
    {
        ...
        ierr = VecSetValue(S->Jcond, i, Jcond, INSERT_VALUES);
        CHKERRQ(ierr);
    }

    ierr = VecAssemblyBegin(S->Jcond);
    CHKERRQ(ierr);
    ierr = VecAssemblyEnd(S->Jcond);
    CHKERRQ(ierr);

    ierr = VecAXPY(S->Jtot, 1.0, S->Jcond);
    CHKERRQ(ierr);
...
static PetscErrorCode append_Jgrav(Ctx *E)
{
    ...
    for (i = ilo_b; i < ihi_b; ++i)
    {
        ...
        ierr = VecSetValue(S->Jgrav, i, Jgrav, INSERT_VALUES);
        CHKERRQ(ierr);
    }

    ierr = VecAssemblyBegin(S->Jgrav);
    CHKERRQ(ierr);
    ierr = VecAssemblyEnd(S->Jgrav);
    CHKERRQ(ierr);

    ierr = VecAXPY(S->Jtot, 1.0, S->Jgrav);
    CHKERRQ(ierr);
...
PetscScalar GetGravitationalHeatFlux(Ctx *E, PetscInt *ind_ptr)
{
    ...
    ierr = VecGetValues(M->pressure_s, 1, ind_ptr, &pres_s);
    CHKERRQ(ierr);
...
PetscErrorCode solve_for_surface_radiation_balance(Ctx *E, PetscReal t)
{
    ...
    ierr = SNESSetOptionsPrefix(snes, "surfrad_");
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-surfrad_snes_stol", "0");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-surfrad_snes_rtol", "1.0E-9");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-surfrad_snes_atol", "1.0");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-surfrad_ksp_rtol", "1.0E-9");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-surfrad_ksp_atol", "1.0E-9");
    CHKERRQ(ierr);
    ...
    /* For solver analysis/debugging/tuning, activate a custom monitor with a flag */
    {
        ...
        ierr = PetscOptionsGetBool(NULL, NULL, "-surfrad_snes_verbose_monitor", &flg, NULL);
        CHKERRQ(ierr);
        if (flg)
        {
...
static PetscErrorCode objective_function_surface_radiation_balance(SNES snes, Vec x, Vec f, void *ptr)
{
    ...
    /* scale residual to physical flux (W/m^2) and recall that
       ...
       the solver settings */
    ierr = VecGetValues(S->Jtot, 1, &ind0, &Jtot0);
    CHKERRQ(ierr);
    res = A->Fatm - Jtot0;
    res *= SC->FLUX; /* W/m^2 */
...
PetscErrorCode solve_for_steady_state_energy_interior(Ctx *E, PetscReal t)
{
    ...
    ierr = SNESSetOptionsPrefix(snes, "steadyint_");
    CHKERRQ(ierr);

    ierr = VecCreate(PETSC_COMM_WORLD, &x);
    CHKERRQ(ierr);
    ierr = VecSetSizes(x, PETSC_DECIDE, numpts_b);
    CHKERRQ(ierr);
    ierr = VecSetFromOptions(x);
    CHKERRQ(ierr);
    ierr = VecDuplicate(x, &r);
    ...
    ierr = SNESSetFunction(snes, r, objective_function_steady_state_energy_interior, E);
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-steadyint_snes_stol", "0");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-steadyint_snes_rtol", "1.0E-9");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-steadyint_snes_atol", "0");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-steadyint_ksp_rtol", "1.0E-9");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-steadyint_ksp_atol", "1.0E-9");
    CHKERRQ(ierr);
    ...
    /* For solver analysis/debugging/tuning, activate a custom monitor with a flag */
    {
        ...
        ierr = PetscOptionsGetBool(NULL, NULL, "-steadyint_snes_verbose_monitor", &flg, NULL);
        CHKERRQ(ierr);
        if (flg)
        {
            ierr = SNESMonitorSet(snes, SNESMonitorVerbose, NULL, NULL);
...
static PetscErrorCode objective_function_steady_state_energy_interior(SNES snes, Vec x, Vec f, void *ptr)
{
    ...
    ierr = VecDuplicate(M->radius_b, &scale_b);
    CHKERRQ(ierr);
    ierr = VecCopy(M->radius_b, scale_b);
    CHKERRQ(ierr);
    ...
    ierr = VecGetValues(S->Etot, 1, &ind0, &E0);
    CHKERRQ(ierr);

    /* compute residual vector */
    /* first part imposes an exact energy balance throughout the mantle */
    ierr = VecShift(S->Etot, -E0);
    CHKERRQ(ierr);
    ierr = VecScale(S->Etot, 1.0 / E0);
    CHKERRQ(ierr);
    ...
    fac2 = 1.0 - fac;
    ierr = VecGetValues(M->radius_b, 1, &ind0, &R0);
    CHKERRQ(ierr);
    ierr = VecGetValues(M->radius_b, 1, &ind1, &R1);
    CHKERRQ(ierr);
    dR = R0 - R1;

    ierr = VecShift(scale_b, -R1);
    CHKERRQ(ierr);
    ierr = VecScale(scale_b, 1.0 / dR);
    CHKERRQ(ierr);
    ierr = VecScale(scale_b, fac);
    CHKERRQ(ierr);
    ierr = VecShift(scale_b, fac2);
    CHKERRQ(ierr);

    ierr = VecPointwiseMult(S->Etot, S->Etot, scale_b);
    CHKERRQ(ierr);

    /* set residual of fluxes */
    ierr = VecCopy(S->Etot, f);
    CHKERRQ(ierr);

    ierr = VecDestroy(&scale_b);
    CHKERRQ(ierr);
...
static PetscErrorCode set_current_state(Ctx *E, PetscReal t)
{
    ...
    ierr = set_reservoir_volatile_content(A, Ap, FC, SC);
    CHKERRQ(ierr);
...
PetscErrorCode set_interior_atmosphere_interface_from_surface_temperature(Ctx *E)
{
    ...
    ierr = set_phase_fraction_staggered(E);
    CHKERRQ(ierr);
    ...
    {
        ierr = set_oxygen_fugacity(A, Ap, SC);
        CHKERRQ(ierr);
    }
...
PetscErrorCode set_current_state_from_solution(Ctx *E, PetscReal t, Vec sol_in)
{
    ...
    CHKERRQ(ierr);
    /* atmosphere */
    ierr = set_partial_pressures_from_solution(E, sol_in);
    CHKERRQ(ierr);
