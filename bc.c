...
PetscErrorCode set_surface_temperature_gradient_update(Ctx *E, Vec rhs)
{
    ...
    CHKERRQ(ierr);
    ierr = VecGetValues(rhs, 1, &ind1, &rhs_below_surf);
    CHKERRQ(ierr);
    ...
    ierr = VecSetValue(rhs, ind0, rhs_surf, INSERT_VALUES);
    CHKERRQ(ierr);
    ...
    ierr = DMDAVecRestoreArrayRead(E->da_b, M->xi_b, &arr_xi_b);
    CHKERRQ(ierr);
...
PetscErrorCode set_cmb_temperature_gradient_update(Ctx *E, Vec rhs)
{
    ...
    ierr = VecGetValues(M->area_b, 1, &ind_cmb, &area_cmb);
    CHKERRQ(ierr);
...
PetscErrorCode set_surface_flux_from_atmosphere(Ctx *E)
{
    ...
    ierr = VecSetValue(S->Jtot, ind0, A->Fatm, INSERT_VALUES);
    CHKERRQ(ierr);
...
PetscErrorCode set_cmb_temperature_constant(Ctx *E)
{
    /* set temperature at core-mantle boundary */
    /* an isothermal bc is naturally accommodated by the cmb bcs, so this
       is only really necessary to set the initial cmb temperature and gradient */
    ...
    CHKERRQ(ierr);
    ierr = DMDAVecGetArrayRead(E->da_s, S->T_s, &arr_T_s);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArrayRead(E->da_b, M->xi_b, &arr_xi_b);
    CHKERRQ(ierr);

    arr_T_b[ihi_b - 1] = P->ic_core_temperature;
    arr_dTdxi_b[ihi_b - 1] = arr_T_b[ihi_b - 1] - arr_T_s[ihi_b - 2];
    arr_dTdxi_b[ihi_b - 1] /= 0.5 * (arr_xi_b[ihi_b - 1] - arr_xi_b[ihi_b - 2]);

    ierr = DMDAVecRestoreArray(E->da_b, S->T, &arr_T_b);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArray(E->da_b, S->dTdxi, &arr_dTdxi_b);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArrayRead(E->da_s, S->T_s, &arr_T_s);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArrayRead(E->da_b, M->xi_b, &arr_xi_b);
    CHKERRQ(ierr);
...
PetscErrorCode set_surface_temperature_constant(Ctx *E)
{
    /* set temperature at surface */
    ...
    arr_T_b[0] = P->ic_surface_temperature;
    arr_dTdxi_b[0] = arr_T_b[0] - arr_T_s[0];
    arr_dTdxi_b[0] /= -0.5 * (arr_xi_b[1] - arr_xi_b[0]);
...
PetscErrorCode set_cmb_temperature_from_cmb_gradient(Ctx *E)
{
    PetscErrorCode ierr;
    Mesh *M = &E->mesh;
    Solution *S = &E->solution;
    ...
    ierr = DMDAVecGetArray(E->da_s, S->T_s, &arr_T_s);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArray(E->da_b, S->T, &arr_T_b);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArray(E->da_b, S->dTdxi, &arr_dTdxi_b);
    ...
    /* use dT/dr at the cmb which is constrained by the core boundary condition */
    arr_T_b[ihi_b - 1] = arr_dTdxi_b[ihi_b - 1] * 0.5 * (arr_xi_b[ihi_b - 1] - arr_xi_b[ihi_b - 2]);
    arr_T_b[ihi_b - 1] += arr_T_s[ihi_b - 2];

    ierr = DMDAVecRestoreArray(E->da_s, S->T_s, &arr_T_s);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArray(E->da_b, S->T, &arr_T_b);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArray(E->da_b, S->dTdxi, &arr_dTdxi_b);
...
PetscErrorCode set_surface_temperature_from_surface_gradient(Ctx *E)
{
    PetscErrorCode ierr;
    Mesh *M = &E->mesh;
    Solution *S = &E->solution;
    ...
    arr_T_b[0] = -arr_dTdxi_b[0] * 0.5 * (arr_xi_b[1] - arr_xi_b[0]);
    arr_T_b[0] += arr_T_s[0];
...
PetscErrorCode get_dpdts_from_lookup(Ctx *E)
{
    ...
    for (i = 0; i < Ap->n_volatiles; ++i)
    {
        ...
        ierr = SetInterp1dValue(Ap->volatile_parameters[i]->TP_interp, A->tsurf, NULL, &A->volatiles[i].dpdt);
        CHKERRQ(ierr);
...
PetscErrorCode solve_dpdts(Ctx *E)
{
    ...
    ierr = SNESSetOptionsPrefix(snes, "atmosts_");
    CHKERRQ(ierr);
    ...
    ierr = SNESSetFunction(snes, r, objective_function_volatile_evolution, E);
    CHKERRQ(ierr);
    ...
    /* Inform the nonlinear solver to generate a finite-difference approximation
       to the Jacobian */
    ierr = PetscOptionsSetValue(NULL, "-atmosts_snes_mf", NULL);
    CHKERRQ(ierr);
    /* get the accuracy from the time stepper and use for atmosts */
    ierr = PetscOptionsGetScalar(NULL, NULL, "-ts_sundials_atol", &atol, NULL);
    CHKERRQ(ierr);
    ierr = PetscOptionsGetScalar(NULL, NULL, "-ts_sundials_rtol", &rtol, NULL);
    CHKERRQ(ierr);
    ierr = SNESSetTolerances(snes, atol, rtol, 0.0, PETSC_DEFAULT, PETSC_DEFAULT);
    CHKERRQ(ierr);
    ...
    /* For solver analysis/debugging/tuning, activate a custom monitor with a flag */
    {
        ...
        ierr = PetscOptionsGetBool(NULL, NULL, "-atmosts_snes_verbose_monitor", &flg, NULL);
        CHKERRQ(ierr);
        if (flg)
        {
