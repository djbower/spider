...
static PetscErrorCode EOSEval_Lookup(EOS eos, PetscScalar P, PetscScalar T, EOSEvalData *eval)
{
  ...
  eval->Cv = 0.0;
  eval->V = 0.0;
  ierr = EOSEvalSetViscosity(eos, eval);
  CHKERRQ(ierr);
...
static PetscErrorCode EOSDestroy_Lookup(EOS eos)
{
  ...
  PetscFunctionBegin;
  ierr = Interp2dDestroy(&lookup->alpha);
  CHKERRQ(ierr);
  ierr = Interp2dDestroy(&lookup->cp);
  CHKERRQ(ierr);
  ierr = Interp2dDestroy(&lookup->dTdPs);
  CHKERRQ(ierr);
...
PetscErrorCode EOSCreate_Lookup(EOS eos)
{
  ...
  ierr = PetscMalloc1(1, (data_EOSLookup **)(&eos->impl_data));
  CHKERRQ(ierr);
