...
PetscErrorCode atmosphere_structs_to_vec( Atmosphere const *A, AtmosphereParameters const * Ap, Vec vec )
{
    ...
    ierr = VecSetValue(vec,9,Ap->H2O_INITIAL,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,10,Ap->H2O_KDIST,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,11,Ap->H2O_KABS,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,12,Ap->H2O_HENRY,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,13,Ap->H2O_HENRY_POW,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,14,Ap->CO2_INITIAL,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,15,Ap->CO2_KDIST,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,16,Ap->CO2_KABS,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,17,Ap->CO2_HENRY,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,18,Ap->CO2_HENRY_POW,INSERT_VALUES);CHKERRQ(ierr);
