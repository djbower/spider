...
PetscErrorCode atmosphere_structs_to_vec( Atmosphere const *A, AtmosphereParameters const * Ap, Vec vec )
{
    ...
    //ierr = VecSetValue(vec,21,M->mantle_mass,INSERT_VALUES);CHKERRQ(ierr);
    ...
    ierr = VecSetValue(vec,24,A->tsurf,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,25,A->tau,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,26,A->p0,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,27,A->dp0dx,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,28,A->m0,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,29,A->tau0,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,30,A->p1,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,31,A->dp1dx,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,32,A->m1,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,33,A->tau1,INSERT_VALUES);CHKERRQ(ierr);
    ierr = VecSetValue(vec,34,A->emissivity,INSERT_VALUES);CHKERRQ(ierr);
