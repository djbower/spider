...
#    CO_atmos_a = data_a[19,:]
    ...
    if 1:
        ...
        h1, = ax0.plot( timeMyr_a, CO2_atmos_a, color=red, linestyle='-', label=r'CO$_2$')
        h2, = ax0.plot( timeMyr_a, H2O_atmos_a, color=blue, linestyle='-', label=r'H$_2$O')
        ax0b = ax0.twinx()
        h3, = ax0b.plot( timeMyr_a, phi_global, color=black, linestyle='--', label=r'Melt, $\phi_g$')
        handle_l = [h1,h2,h3]
    ...
    if 1:
        ...
        h1, = ax1.plot( timeMyr_a, (CO2_liquid_kg_a+CO2_solid_kg_a) / CO2_total_kg, color=red, linestyle='-', label=r'CO$_2$ interior' )
        h2, = ax1.plot( timeMyr_a, CO2_atmos_kg_a / CO2_total_kg, color=red, linestyle='--', label=r'CO$_2$ atmos' )
        #h2b, = ax1.semilogx( timeMyr_a, CO2_escape_kg_a / CO2_total_kg, color=red, linestyle=':', label='Escape' )
        h3, = ax1.plot( timeMyr_a, (H2O_liquid_kg_a+H2O_solid_kg_a) / H2O_total_kg, color=blue, linestyle='-', label=r'H$_2$O interior' )
        h4, = ax1.plot( timeMyr_a, H2O_atmos_kg_a / H2O_total_kg, color=blue, linestyle='--', label=r'H$_2$O atmos')
    ...
    # figure c
    ##########
    title = r'\textbf{(c) Surface and mantle temp}'
    ylabel = '$T$ (K)'
    yticks = range(500,4001,500)
    h1, = ax2.plot( timeMyr_a, temperature_surface_a, 'k-', label=r'Surface temp, $T_s$' )
    h2, = ax2.plot( timeMyr_a, temperature_mantle_a, 'k--', label=r'Mantle temp, $T_m$' )
    #ax2b = ax2.twinx()
    ...
    if 1:
        ...
        ylabel = '$F_{atm}$'
        yticks = (1.0E3,1.0E4,1.0E5,1.0E6,5.0E6)
        h1, = ax3.semilogy( timeMyr_a, Fatm,'k', )
