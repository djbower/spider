...
PetscErrorCode setup_ctx(Ctx* ctx)
{
  ...
  */
  ...
  ctx->solution.dSdr                = ctx->solution.solutionVecs_b[8];
  ctx->solution.dSliqdr             = ctx->solution.solutionVecs_b[9];
  ctx->solution.dSsoldr             = ctx->solution.solutionVecs_b[10];
  ctx->solution.dTdPs               = ctx->solution.solutionVecs_b[11];
  ctx->solution.dTdPs_mix           = ctx->solution.solutionVecs_b[12];
  ctx->solution.dTdrs               = ctx->solution.solutionVecs_b[13];
  ctx->solution.dTdrs_mix           = ctx->solution.solutionVecs_b[14];
  ctx->solution.Etot                = ctx->solution.solutionVecs_b[15];
  ctx->solution.fusion              = ctx->solution.solutionVecs_b[16];
  ctx->solution.fusion_curve        = ctx->solution.solutionVecs_b[17];
  ctx->solution.fusion_curve_temp   = ctx->solution.solutionVecs_b[18];
  ctx->solution.fusion_rho          = ctx->solution.solutionVecs_b[19];
  ctx->solution.fusion_temp         = ctx->solution.solutionVecs_b[20];
  ...
  ctx->solution.fusion_s            = ctx->solution.solutionVecs_s[0];
  ctx->solution.fusion_curve_s      = ctx->solution.solutionVecs_s[1];
  ctx->solution.fusion_curve_temp_s = ctx->solution.solutionVecs_s[2];
  ctx->solution.fusion_temp_s       = ctx->solution.solutionVecs_s[3];
  ctx->solution.lhs_s               = ctx->solution.solutionVecs_s[4];
  ctx->solution.liquidus_rho_s      = ctx->solution.solutionVecs_s[5];
  ctx->solution.liquidus_s          = ctx->solution.solutionVecs_s[6];
  ctx->solution.liquidus_temp_s     = ctx->solution.solutionVecs_s[7];
  ...
  ctx->solution.solidus_s           = ctx->solution.solutionVecs_s[11];
  ctx->solution.solidus_rho_s       = ctx->solution.solutionVecs_s[12];
...
PetscErrorCode set_matprop_and_flux( Ctx *E )
{
    ...
    for(i=ilo; i<ihi; ++i){ // Note that these correspond to basic nodes being updated, but we also assume an ordering on the staggered nodes!
      ...
      if (gphi > 0.5 ){
        ...
        arr_dTdPs[i] += fwtl * get_val2d( &L->dTdPs, arr_pres[i], arr_S_b[i] );
