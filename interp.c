...
PetscErrorCode Interp1dCreateAndSet(const char *filename, Interp1d *interp_ptr, PetscScalar xconst, PetscScalar yconst)
{
    ...
    ierr = PetscMalloc1(1, interp_ptr);
    CHKERRQ(ierr);
    interp = *interp_ptr;
    ...
    while (fgets(string, sizeof(string), fp) != NULL)
    {
        ...
        if (i == 0)
        {
            ...
            ierr = PetscMalloc1(NX, &interp->xa);
            CHKERRQ(ierr);
            ierr = PetscMalloc1(NX, &interp->ya);
            CHKERRQ(ierr);
        }
    ...
    for (i = 0; i < NX; i++)
    {
        ierr = PetscPrintf(PETSC_COMM_SELF, "[%D] %d %f\n", rank, i, (double)interp->xa[i]);
        CHKERRQ(ierr);
    }
    for (i = 0; i < NX; i++)
    {
        ierr = PetscPrintf(PETSC_COMM_SELF, "[%D] %d %f\n", rank, i, (double)interp->ya[i]);
        CHKERRQ(ierr);
    }
#endif
...
PetscErrorCode Interp2dCreateAndSet(const char *filename, Interp2d *interp_ptr, PetscScalar xconst, PetscScalar yconst, PetscScalar zconst)
{
    ...
    while (fgets(string, sizeof(string), fp) != NULL)
    {
        ...
        if (i == 0)
        {
            ...
            ierr = PetscMalloc1(NX, &interp->xa);
            CHKERRQ(ierr);
            ierr = PetscMalloc1(NY, &interp->ya);
            CHKERRQ(ierr);
            ierr = Make2DPetscScalarArray(NX, NY, &interp->za);
            CHKERRQ(ierr);
        }
...
PetscErrorCode Interp1dDestroy(Interp1d *interp_ptr)
{
    ...
    ierr = PetscFree(interp->xa);
    CHKERRQ(ierr);
    ierr = PetscFree(interp->ya);
    CHKERRQ(ierr);

    ierr = PetscFree(*interp_ptr);
    CHKERRQ(ierr);
    *interp_ptr = NULL;
...
PetscErrorCode Interp2dDestroy(Interp2d *interp_ptr)
{
    ...
    for (i = 0; i < NX; i++)
    {
        ierr = PetscFree(interp->za[i]);
        CHKERRQ(ierr);
    }
    ierr = PetscFree(interp->za);
    CHKERRQ(ierr);
