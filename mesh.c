...
PetscErrorCode set_mesh(Ctx *E)
{
    ...
    {
        ierr = GetRadiusFromMassCoordinate(E);
        CHKERRQ(ierr);
    }
    else
    {
        ierr = RadiusIsMassCoordinate(E);
        CHKERRQ(ierr);
    }

    /* these all currently use the Adams-Williamson EOS, but it could be
       easily generalised */
    ierr = SetMeshPressureFromRadius(P->eos_mesh, da_b, M->radius_b, M->pressure_b);
    CHKERRQ(ierr);
    ierr = SetMeshPressureGradientFromRadius(P->eos_mesh, da_b, M->radius_b, M->dPdr_b);
    CHKERRQ(ierr);
    ierr = SetMeshPressureFromRadius(P->eos_mesh, da_s, M->radius_s, M->pressure_s);
    CHKERRQ(ierr);
    ierr = SetMeshPressureGradientFromRadius(P->eos_mesh, da_s, M->radius_s, M->dPdr_s);
    CHKERRQ(ierr);
    ierr = SetMeshMass(P->eos_mesh, E);
    CHKERRQ(ierr);

    /* geometry terms without 4*pi prefactor */
    ierr = SetMeshSphericalArea(da_b, M->radius_b, M->area_b);
    CHKERRQ(ierr);
    ierr = SetMeshSphericalArea(da_s, M->radius_s, M->area_s);
    CHKERRQ(ierr);
    ierr = SetMeshSphericalVolume(E, M->radius_b, M->volume_s);
    CHKERRQ(ierr);
...
static PetscErrorCode SetMeshRegular(Ctx *E)
{
    ...
    CHKERRQ(ierr);
    ihi_b = ilo_b + w_b;
    ierr = DMDAVecGetArray(da_b, M->xi_b, &arr);
    CHKERRQ(ierr);
    for (i = ilo_b; i < ihi_b; ++i)
    {
        arr[i] = P->radius * P->coresize - (numpts_b - 1 - i) * dx_b;
    }
    ierr = DMDAVecRestoreArray(da_b, M->xi_b, &arr);
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ihi_s = ilo_s + w_s;
    ierr = DMDAVecGetArray(da_s, M->xi_s, &arr);
    CHKERRQ(ierr);
    for (i = ilo_s; i < ihi_s; ++i)
    {
        arr[i] = P->radius * P->coresize - 0.5 * dx_b - (numpts_s - 1 - i) * dx_b;
    }
    ierr = DMDAVecRestoreArray(da_s, M->xi_s, &arr);
    CHKERRQ(ierr);
...
static PetscErrorCode SetMeshSphericalArea(DM da, Vec radius, Vec area)
{
    ...
    PetscFunctionBeginUser;
    ierr = DMDAGetCorners(da, &ilo, 0, 0, &w, 0, 0);
    CHKERRQ(ierr);
    ihi = ilo + w;
    ierr = DMDAVecGetArrayRead(da, radius, &arr_radius);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArray(da, area, &arr_area);
    CHKERRQ(ierr);
    for (i = ilo; i < ihi; ++i)
    {
    ...
    ierr = DMDAVecRestoreArrayRead(da, radius, &arr_radius);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArray(da, area, &arr_area);
    CHKERRQ(ierr);
    PetscFunctionReturn(0);
}
...
static PetscErrorCode SetMeshSphericalVolume(Ctx *E, Vec radius, Vec volume)
{
    ...
    ilo = ilo_s;
    ihi = ihi_s;
    ierr = DMGlobalToLocalBegin(da_b, radius, INSERT_VALUES, radius_local);
    CHKERRQ(ierr);
    ierr = DMGlobalToLocalEnd(da_b, radius, INSERT_VALUES, radius_local);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArrayRead(da_b, radius_local, &arr_radius);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArray(da_s, volume, &arr_volume);
    CHKERRQ(ierr);
    for (i = ilo; i < ihi; ++i)
    {
    ...
    ierr = DMDAVecRestoreArrayRead(da_b, radius_local, &arr_radius);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArray(da_s, volume, &arr_volume);
    CHKERRQ(ierr);
    PetscFunctionReturn(0);
}
...
static PetscErrorCode SetMeshPressureFromRadius(const EOS eos, DM da, Vec radius, Vec pressure)
{
    ...
    CHKERRQ(ierr);
    ihi = ilo + w;
    ierr = DMDAVecGetArrayRead(da, radius, &arr_r);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArray(da, pressure, &arr_p);
    CHKERRQ(ierr);
    for (i = ilo; i < ihi; ++i)
    {
        ierr = EOSAdamsWilliamsonGetPressureFromRadius(eos, arr_r[i], &arr_p[i]);
        CHKERRQ(ierr);
    }
    ierr = DMDAVecRestoreArrayRead(da, radius, &arr_r);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArray(da, pressure, &arr_p);
    CHKERRQ(ierr);
    PetscFunctionReturn(0);
}
...
static PetscErrorCode SetMeshPressureGradientFromRadius(const EOS eos, DM da, Vec radius, Vec pressureg)
{
    ...
    CHKERRQ(ierr);
    ihi = ilo + w;
    ierr = DMDAVecGetArray(da, pressureg, &arr_pg);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArrayRead(da, radius, &arr_r);
    CHKERRQ(ierr);
    for (i = ilo; i < ihi; ++i)
    {
        ierr = EOSAdamsWilliamsonGetPressureGradientFromRadius(eos, arr_r[i], &arr_pg[i]);
        CHKERRQ(ierr);
    }
    ierr = DMDAVecRestoreArray(da, pressureg, &arr_pg);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArrayRead(da, radius, &arr_r);
    CHKERRQ(ierr);
    PetscFunctionReturn(0);
...
static PetscErrorCode SetMeshMass(const EOS eos, Ctx *E)
{
    ...
    PetscFunctionBeginUser;
    ierr = DMDAGetCorners(E->da_b, &ilo, 0, 0, &w, 0, 0);
    CHKERRQ(ierr);
    ihi = ilo + w;

    ierr = DMDAVecGetArray(E->da_b, M->radius_b, &arr_r);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArray(E->da_s, M->mass_s, &arr_m);
    CHKERRQ(ierr);

    for (i = ilo; i < ihi - 1; ++i)
    {
        ierr = EOSAdamsWilliamsonGetMassWithinShell(eos, arr_r[i], arr_r[i + 1], &arr_m[i]);
        CHKERRQ(ierr);
    }
    /* total mantle mass */
    ierr = EOSAdamsWilliamsonGetMassWithinShell(eos, arr_r[ilo], arr_r[ihi - 1], &M->mantle_mass);
    CHKERRQ(ierr);

    ierr = DMDAVecRestoreArrayRead(E->da_b, M->radius_b, &arr_r);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArrayRead(E->da_s, M->mass_s, &arr_m);
    CHKERRQ(ierr);
...
static PetscErrorCode RadiusIsMassCoordinate(Ctx *E)
{
    ...
    ierr = VecCopy(M->xi_b, M->radius_b);
    CHKERRQ(ierr);
    ierr = VecCopy(M->xi_s, M->radius_s);
    CHKERRQ(ierr);
    ierr = VecSet(M->dxidr_b, 1.0);
    CHKERRQ(ierr);
...
static PetscErrorCode GetRadiusFromMassCoordinate(Ctx *E)
{
    ...
    ierr = SNESSetOptionsPrefix(snes, "mass_coord_");
    CHKERRQ(ierr);

    /* convert to DMComposite? */
    ierr = VecCreate(PETSC_COMM_WORLD, &x);
    CHKERRQ(ierr);
    ierr = VecSetSizes(x, PETSC_DECIDE, numpts_b + numpts_s);
    CHKERRQ(ierr);
    ...
    ierr = SNESSetFunction(snes, r, EOSAdamsWilliamson_ObjectiveFunctionRadius, E);
    CHKERRQ(ierr);
    ...
    ierr = SNESSetJacobian(snes, J, J, EOSAdamsWilliamson_JacobianRadius, E);
    CHKERRQ(ierr);
    ...
    ierr = PetscOptionsSetValue(NULL, "-mass_coord_snes_rtol", "1.0e-6");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-mass_coord_snes_atol", "1.0e-6");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-mass_coord_ksp_rtol", "1.0e-6");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-mass_coord_ksp_atol", "1.0e-6");
    CHKERRQ(ierr);
    ...
    /* For solver analysis/debugging/tuning, activate a custom monitor with a flag */
    {
        ...
        ierr = PetscOptionsGetBool(NULL, NULL, "-mass_coord_snes_verbose_monitor", &flg, NULL);
        CHKERRQ(ierr);
        if (flg)
        {
    ...
    ierr = DMDAVecGetArray(E->da_b, M->radius_b, &radius);
    CHKERRQ(ierr);
    for (i = 0; i < numpts_b; ++i)
    {
        if (xx[i] < 0.0)
    ...
    ierr = DMDAVecRestoreArray(E->da_b, M->radius_b, radius);
    CHKERRQ(ierr);

    /* extract solution for staggered radius from solution vec */
    ierr = DMDAVecGetArray(E->da_s, M->radius_s, &radius);
    CHKERRQ(ierr);
    for (i = numpts_b; i < numpts_b + numpts_s; ++i)
    {
        if (xx[i] < 0.0)
    ...
    ierr = DMDAVecRestoreArray(E->da_s, M->radius_s, radius);
    CHKERRQ(ierr);
    ...
    ierr = DMDAVecGetArray(E->da_b, M->dxidr_b, &dxidr);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArrayRead(E->da_b, M->radius_b, &radius);
    CHKERRQ(ierr);
    ierr = DMDAVecGetArrayRead(E->da_b, M->xi_b, &xi);
    CHKERRQ(ierr);
    for (i = 0; i < numpts_b; ++i)
    {
        EOSAdamsWilliamsonMassCoordinateSpatialDerivative(eos, radius[i], xi[i], &dxidr[i]);
        CHKERRQ(ierr);
    }
    ierr = DMDAVecRestoreArray(E->da_b, M->dxidr_b, &dxidr);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArrayRead(E->da_b, M->radius_b, &radius);
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArrayRead(E->da_b, M->xi_b, &xi);
    CHKERRQ(ierr);
