...
PetscErrorCode set_initial_condition(Ctx *E, Vec sol)
{
    ...
    {
        ierr = solve_for_surface_radiation_balance(E, P->t0);
        CHKERRQ(ierr);
...
static PetscErrorCode set_ic_interior(Ctx *E, Vec sol)
{
    ...
    if (P->IC_INTERIOR == 1)
    {
        ierr = set_ic_interior_default(E, sol);
        CHKERRQ(ierr);
    }
    else if (P->IC_INTERIOR == 2)
    {
        ierr = set_ic_interior_from_file(E, sol);
        CHKERRQ(ierr);
        ierr = set_start_time_from_file(P, P->ic_interior_filename);
        CHKERRQ(ierr);
        ierr = set_start_stepmacro_from_file(P, P->ic_interior_filename);
        CHKERRQ(ierr);
    }
    else if (P->IC_INTERIOR == 3)
    {
        ierr = set_ic_interior_from_phase_boundary(E, sol);
        CHKERRQ(ierr);
    }
    else
    {
...
static PetscErrorCode set_ic_interior_from_file(Ctx *E, Vec sol)
{
    ...
    ierr = set_ic_from_file(E, sol, P->ic_interior_filename, arr, 2);
    CHKERRQ(ierr);
...
static PetscErrorCode set_ic_atmosphere_from_file(Ctx *E, Vec sol)
{
    ...
    if (Ap->n_volatiles)
    {
        ierr = set_ic_from_file(E, sol, Ap->ic_atmosphere_filename, arr1, 1);
        CHKERRQ(ierr);
    }

    if (Ap->n_reactions)
    {
        ierr = set_ic_from_file(E, sol, Ap->ic_atmosphere_filename, arr2, 1);
        CHKERRQ(ierr);
    }
    ...
    if (fp)
    {
        ...
        if (buffer)
        {
            if (fread(buffer, 1, length, fp) != length)
                SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_FILE_READ, "fread() error");
        }
        else
            SETERRQ(PETSC_COMM_WORLD, PETSC_ERR_MEM, "malloc error");
        fclose(fp);
        buffer[length] = '\0';
    }
...
#endif
                    /* add value to vec */
                    VecSetValue(invec, i, val, INSERT_VALUES);
                    CHKERRQ(ierr);
                }
            }
        }
...
static PetscErrorCode set_ic_interior_from_phase_boundary(Ctx *E, Vec sol)
{
    ...
    CHKERRQ(ierr);
    ierr = DMDAVecGetArrayRead(E->da_s, M->pressure_s, &arr_pres_s);
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ierr = DMDAVecRestoreArrayRead(da_s, M->pressure_s, &arr_pres_s);
    CHKERRQ(ierr);
...
static PetscErrorCode set_ic_atmosphere(Ctx *E, Vec sol)
{
    ...
    if (Ap->n_volatiles)
    {
        ...
        {
            ierr = set_ic_atmosphere_pseudo_volatiles(E, sol);
            CHKERRQ(ierr);
        }
        ...
        {
            ierr = set_ic_atmosphere_from_initial_total_abundance(E, sol);
            CHKERRQ(ierr);
        }
        ...
        else if (Ap->IC_ATMOSPHERE == 2)
        {
            ...
            ierr = set_ic_atmosphere_from_file(E, sol);
            CHKERRQ(ierr);
        }
        ...
        {
            ierr = set_ic_atmosphere_from_partial_pressure(E, sol);
            CHKERRQ(ierr);
        }
        ...
        {
            ierr = set_ic_atmosphere_from_ocean_moles(E, sol);
            CHKERRQ(ierr);
        }
        ...
        ierr = set_partial_pressures_from_solution(E, sol);
        CHKERRQ(ierr);
        ...
        ierr = conform_atmosphere_parameters_to_ic(E);
        CHKERRQ(ierr);

        ierr = set_solution_from_partial_pressures(E, sol);
        CHKERRQ(ierr);
    }
...
static PetscErrorCode print_ocean_masses(Ctx *E)
{
    ...
    if (FLAG_H2)
    {
        ...
        ierr = PetscPrintf(PETSC_COMM_WORLD, "%-30s %-15.6g\n", "Equivalent present-day mass of ocean water from H2 (non-dimensional)", (double)tmass_H2);
        CHKERRQ(ierr);
        ierr = PetscPrintf(PETSC_COMM_WORLD, "%-30s %-15.6g\n", "Equivalent atmospheric pressure of H2 (bar)", (double)p_H2);
        CHKERRQ(ierr);
    }
    ...
    if (FLAG_H2O)
    {
        ...
        ierr = PetscPrintf(PETSC_COMM_WORLD, "%-30s %-15.6g\n", "Equivalent present-day mass of ocean water from H2O (non-dimensional)", (double)tmass_H2O);
        CHKERRQ(ierr);
        ierr = PetscPrintf(PETSC_COMM_WORLD, "%-30s %-15.6g\n", "Equivalent atmospheric pressure of H2O (bar)", (double)p_H2O);
        CHKERRQ(ierr);
    }
    ...
    if (FLAG_CO)
    {
        ...
        ierr = PetscPrintf(PETSC_COMM_WORLD, "%-30s %-15.6g\n", "Equivalent present-day mass of ocean water from CO (non-dimensional)", (double)tmass_CO);
        CHKERRQ(ierr);
        ierr = PetscPrintf(PETSC_COMM_WORLD, "%-30s %-15.6g\n", "Equivalent atmospheric pressure of CO (bar)", (double)p_CO);
        CHKERRQ(ierr);
    }
    ...
    if (FLAG_CO2)
    {
        ...
        ierr = PetscPrintf(PETSC_COMM_WORLD, "%-30s %-15.6g\n", "Equivalent present-day mass of ocean water from CO2 (non-dimensional)", (double)tmass_CO2);
        CHKERRQ(ierr);
        ierr = PetscPrintf(PETSC_COMM_WORLD, "%-30s %-15.6g\n", "Equivalent atmospheric pressure of CO2 (bar)", (double)p_CO2);
        CHKERRQ(ierr);
    }
...
static PetscErrorCode set_ic_atmosphere_from_initial_total_abundance(Ctx *E, Vec sol)
{
    ...
    ierr = solve_for_initial_partial_pressure(E);
    CHKERRQ(ierr);
...
static PetscErrorCode solve_for_initial_partial_pressure(Ctx *E)
{
    ...
    ierr = SNESSetOptionsPrefix(snes, "atmosic_");
    CHKERRQ(ierr);

    ierr = VecCreate(PETSC_COMM_WORLD, &x);
    CHKERRQ(ierr);
    /* Vector with one dof per volatile species, and one dof per volatile reaction */
    ierr = VecSetSizes(x, PETSC_DECIDE, Ap->n_volatiles + Ap->n_reactions);
    CHKERRQ(ierr);
    ...
    ierr = SNESSetFunction(snes, r, objective_function_initial_partial_pressure, E);
    CHKERRQ(ierr);
    ...
    ierr = PetscOptionsSetValue(NULL, "-atmosic_snes_type", "newtontr");
    CHKERRQ(ierr);
    /* Inform the nonlinear solver to generate a finite-difference approximation
       to the Jacobian */
    ierr = PetscOptionsSetValue(NULL, "-atmosic_snes_mf", NULL);
    CHKERRQ(ierr);
    /* Turn off convergence based on step size */
    ierr = PetscOptionsSetValue(NULL, "-atmosic_snes_stol", "0");
    CHKERRQ(ierr);
    /* Turn off convergenced based on trust region tolerance */
    ierr = PetscOptionsSetValue(NULL, "-atmosic_snes_trtol", "0");
    CHKERRQ(ierr);
    ...
    ierr = PetscOptionsSetValue(NULL, "-atmosic_snes_tr_delta0", "10.0");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-atmosic_snes_rtol", "1.0e-9");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-atmosic_snes_atol", "1.0e-9");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-atmosic_ksp_rtol", "1.0e-9");
    CHKERRQ(ierr);
    ierr = PetscOptionsSetValue(NULL, "-atmosic_ksp_rtol", "1.0e-9");
    CHKERRQ(ierr);
    ...
    /* For solver analysis/debugging/tuning, activate a custom monitor with a flag */
    {
        ...
        ierr = PetscOptionsGetBool(NULL, NULL, "-atmosic_snes_verbose_monitor", &flg, NULL);
        CHKERRQ(ierr);
        if (flg)
        {
